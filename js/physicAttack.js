function getBaseDamage(isCrit)
{
	...
	let groupAMul = groupAMultiplier(isCrit);
	let groupBMul = groupBMultiplier(isCrit);
	...
	for(let i = 0; i <= 2;i++)
	{
		if(SkillSearch(skill_TK_MILD_WIND) == 0)
			baseDamage[i] = Math.floor((StatusATK * 2) + groupAMul[i] + groupBMul[i]) * (1 + (PATK / 100)) + MasteryATK + BuffATK;
		else
			baseDamage[i] = Math.floor((StatusATK * (1 + (PropertyMultiplier/100)) * 2) + groupAMul[i] + groupBMul[i]) * (1 + (PATK / 100)) + MasteryATK + BuffATK;
...
function getBaseDamage2(isCrit)
{
	let StatusATK = getStatATK();
	let MasteryATK = getMasteryATK();
	let BuffATK = 0;
	...
	let groupAMul = groupAMultiplier2(isCrit);
	let groupBMul = groupBMultiplier2(isCrit);
	...
	{
		if(SkillSearch(skill_TK_MILD_WIND) == 0)
			baseDamage[i] = Math.floor((StatusATK * 1) + groupAMul[i] + groupBMul[i]) * (1 + (PATK / 100)) + MasteryATK + BuffATK;
		else
...
function getFinalDamage(finalDamage,sk,SkillLevel,isCrit)
{
	for(let i = 0; i <= 2;i++)
	{
		...
		if(sk.id == skill_CR_GRAND_CROSS)
			finalDamage[i] /= 2;
		//CriticalMultiplier
		if(isCrit){
			if(sk.id != 0 || sk.id != skill_RUN_GIANT_GROWTH || sk.id != skill_TH_DOUBLE_ATTACK)//for skills, crit multiplier are halved
		...
		//ranged multiplier
		if(sk.range[SkillLevel -1] > 4 || 
		  (sk.id == skill_RUN_WIND_CUTTER && PATCH == 2))
		{
			//ranged multiplier
			finalDamage[i] = Math.floor(finalDamage[i] * (1 + (getRangedMultiplier()/100)))
			//ranged reduction
		...
		else
		{
			switch(sk.id)
			{
				case skill_LK_CLASHING_SPIRAL:
					finalDamage[i] *= (getBaseDamageNoMastery()[i] + ItemOBJ[n_A_Equip[eq_WEAPON]][itm_WEIGHT]) * 0.7 * (1 + (0.15 * (2 - n_B[en_SIZE]))) + getMasteryATK();
					finalDamage[i] *= Math.floor(sk.skillFormula(SkillLevel))/100;
					break;
				...
				case skill_RAN_ICEBOUND_TRAP:
					...
					finalDamage[i] *= Math.floor(sk.skillFormula(SkillLevel))/100;
					finalDamage[i] += (((SkillLevel * n_A_DEX) + (n_A_INT * 5)) * (1.5 + (n_A_BaseLV / 100))) * ((SkillSearch(skill_RAN_RESEARCH_TRAP) * 20) / 100);
					break;
				case skill_RAN_CLUSTER_BOMB:
					...
					finalDamage[i] *= Math.floor(sk.skillFormula(SkillLevel))/100;
					finalDamage[i] += (((SkillLevel * n_A_DEX) + (n_A_INT * 5)) * (1.5 + (n_A_BaseLV / 100))) * ((SkillSearch(skill_RAN_RESEARCH_TRAP) * 20) / 50);
					break;
		...
		//Damage Multiplier
		finalDamage[i] = Math.floor(finalDamage[i] * (1 + (getDamageMultiplier()/100)));
		...
		else
		{
			...
			// finalDamage[i] -= (n_B[en_HARDDEF] + getSoftDEF(true));
			finalDamage[i] -= (n_B[en_HARDDEF] + getSoftDEF(true))/2;//good for Gfist
			// finalDamage[i] -= n_B[en_HARDDEF];
			// finalDamage[i] -= (getSoftDEF(true));
		}
		...
		finalDamage[i] = Math.floor(finalDamage[i] * (1 + (getFinalDamageMultiplier(sk.id)/100)));

		//Final Damage Reduction
		finalDamage[i] = Math.floor(finalDamage[i] * (1 + (getFinalDamageReduction()/100)));
		...
		switch(sk.id)
		{
			case 0://for basic attacks only
				if(SkillSearch(skill_RUN_ENCHANT_BLADE))
					finalDamage[i] += RUN_ENCHANT_BLADE.skillFormula(SkillSearch(skill_RUN_ENCHANT_BLADE)) + BK_n_A_MATK[i];
			...
			case skill_SUR_GATE_OF_HELL:
				...
				if(formElements["SkillSubNum"].checked)
					finalDamage[i] += (n_A_MaxHP - remainingHP) + (remainingSP * (1 + (0.2 * SkillLevel))) + (n_A_BaseLV * 10);
				else
					finalDamage[i] += (n_A_MaxHP - remainingHP) + (n_A_MaxSP * (1 + (0.2 * SkillLevel))) + (n_A_BaseLV * 40);
				break;
			case skill_SUR_CRESCENT_ELBOW:
				finalDamage[i] += parseInt(formElements["SkillSubNum"].value) * (1 + (0.2 * SkillLevel));
				break;			
			default:
				break;
		}
		//aura blade should be in post def damage
		finalDamage[i] += LK_AURA_BLADE.skillFormula(SkillSearch(skill_LK_AURA_BLADE));

		//hit divisibility
		finalDamage[i] = Math.floor(finalDamage[i] / eval(sk.hitDivisibility)) * eval(sk.hitDivisibility);
		...
		if(sk.id == skill_SUR_KNUCKLE_ARROW)
			if(formElements["SkillSubNum"].checked)
				finalDamage[i] += getFinalDamage(getBaseDamage(0),SUR_KNUCKLE_ARROW_KNOCKBACK,SkillLevel,0)[i];
...
function getSkill()
{
	...
	// return {multiplier : (400+n_A_AGI) * (n_A_BaseLV/100),numHit:3,totalHit:3};
	// return {multiplier : 220 * (n_A_BaseLV/100),numHit:2,totalHit:2};
	return {multiplier : 100,numHit:1,totalHit:1};
	// return {multiplier : 500,numHit:1,totalHit:2};
}
function groupAMultiplier(isCrit)
{
	...
	for(let i = 0; i <= 2;i++)
		groupAMul[i] = Math.floor((WeaponATK[i] + ExtraATK) * (Type2ATKMultiplier / 100) * (PropertyMultiplier / 100));
	return groupAMul;
}
...
function getType2ATKMultiplier()
{
	...
	if(not_use_card == 0)
		bAtkRate += StPlusCard( bon_ATK_MUL);
	// Attack Mod is physical mod *
	
// Equipment
	// "Thanatos' Dolor Hat"
	if(EquipNumSearch( 1637 ) && (n_A_HEAD_DEF_PLUS > 6))
		bAtkRate += 5;
	//"Officer's Cap"
	if(EquipNumSearch(1682) && (n_A_HEAD_DEF_PLUS > 6))
		bAtkRate += 5;
	//"Dog Cap" || "Feathered Tricorn" || General's Helmet
	if((EquipNumSearch(1702) || EquipNumSearch(1860) || EquipNumSearch(1942)) && (n_A_HEAD_DEF_PLUS >= 9))
		bAtkRate += 5;
	//Armor of Sixtus the Mighty
	if( EquipNumSearch( 2037 ) )
		bAtkRate += 2 * Math.floor(n_A_BODY_DEF_PLUS / 3);
	//Prime only ? 
	...
	if(EquipNumSearch(2142))
		bAtkRate += Math.floor(n_A_SHOULDER_DEF_PLUS / 2);
...
//shadows
	...
	if( EquipNumSearch(1840) )
		bAtkRate += n_A_SHADOW_WEAPON_DEF_PLUS;	
	//test
...
function groupBMultiplier(isCrit)
{
	let WeaponATK = getWeaponATK(isCrit);
	let ExtraATK = getExtraATK();
	...
	let skillBonus = 0;
	if((n_A_ActiveSkill != skill_AS_VENOM_SPLASHER && 
		...
		skillBonus += AX_ENCHANT_DEADLY_POISON.skillFormula(SkillSearch(skill_AX_ENCHANT_DEADLY_POISON));
	if((SkillSearch(skill_SUR_GENTLE_TOUCH_CHANGE) || (acolyteBuffs[ksPPChange] > 0)) && (PATCH >= 2))
	{
		if(SkillSearch(skill_SUR_GENTLE_TOUCH_CHANGE))
			skillBonus += SkillSearch(skill_SUR_GENTLE_TOUCH_CHANGE);
		else
			skillBonus += acolyteBuffs[ksPPChange];
	}
	...
	skillBonus += TKM_STELLAR_WRATH.skillFormula(SkillSearch(skill_TKM_STELLAR_WRATH));

	let groupBMul = new Array();
	for(let i = 0; i <= 2;i++)
		groupBMul[i] = Math.floor((WeaponATK[i] + ExtraATK) * (1 + (RaceMultiplier / 100)) * (1 + (SizeMultiplier / 100)) * (1 + (TargetPropertyMultiplier / 100)) * (1 + (MonsterMultiplier / 100)) * (1 + (ATKMultiplier / 100)) * (PropertyMultiplier / 100) * (1 + (skillBonus / 100)));
	return groupBMul;
}
...
{
	let raceMultiplier = StPlusCalc2( bon_DMG_RC_FORMLESS + n_B[en_RACE]) + StPlusEnchant( bon_DMG_RC_FORMLESS + n_B[en_RACE] );
	if(not_use_card == 0)
	...
	raceMultiplier += SA_DRAGONOLOGY.skillFormula(SkillSearch(skill_SA_DRAGONOLOGY));
	
	if( EquipNumSearch( 1335 ) && n_A_HEAD_DEF_PLUS >= 5 && n_B[en_RACE]== race_DEMI_HUMAN)
	{ // Cat Ear Beret
		for ( var i = 5; i <= 12; i++ )
	...
	if(EquipNumSearch(2057) && (n_B[en_RACE]== race_DEMI_HUMAN || n_B[en_RACE]== race_BRUTE))
	{
		if(n_A_Weapon_ATKplus >= 7)
			raceMultiplier += 15;
	...
	//Four Mirrors
	if(EquipNumSearch(2075) && n_A_Weapon_ATKplus >= 11 && (n_B[en_RACE]== race_UNDEAD || n_B[en_RACE]== race_DEMON))
		raceMultiplier += 20;
	//Demon's Shot
	if( EquipNumSearch(2078) && n_A_Weapon_ATKplus >= 11 && (n_B[en_RACE]== race_UNDEAD || n_B[en_RACE]== race_DEMON))
		raceMultiplier += 15;
	// "Headband Beret"
	...
	if( (EquipNumSearch( 1076 ) || EquipNumSearch( 1077 ) || EquipNumSearch( 1081 ) ||
		  ...
		  EquipNumSearch( 1101 ) || EquipNumSearch( 1102 ) || EquipNumSearch( 1103 )) && n_A_Weapon_ATKplus >= 6 && n_B[en_RACE]== race_DEMI_HUMAN) {
		raceMultiplier += Math.pow(Math.min(10, n_A_Weapon_ATKplus-4), 2);
	}
	if( (EquipNumSearch( 1080 ) || EquipNumSearch( 1087 ) || EquipNumSearch( 1098 )) && n_A_Weapon_ATKplus >= 6 && n_B[en_RACE]== race_DEMI_HUMAN) {
		raceMultiplier += Math.pow(Math.min(10, n_A_Weapon_ATKplus-3), 2);
	}
	if( (EquipNumSearch( 1637 ) ) && (n_B[en_RACE]== race_ANGEL || n_B[en_RACE]== race_DRAGON))
	{// "Thanatos' Dolor Hat"
	...
	//Armor of Sixtus the Mighty
	if( EquipNumSearch( 2037 ) && n_A_BODY_DEF_PLUS >= 9 && (n_B[en_RACE]== race_BRUTE || n_B[en_RACE]== race_DEMON))
		raceMultiplier += 10;
...
{
	let sizeMultiplier = StPlusCalc2( bon_DMG_SIZ_SMALL + n_B[en_SIZE]) + StPlusEnchant( bon_DMG_SIZ_SMALL + n_B[en_SIZE] );
	if(not_use_card == 0)
	...
	{
		//Mutant Plaga Card
		if(CardNumSearch(847) && n_A_WeaponType == weapTyp_KNUCKLE && n_B[en_SIZE] == siz_LARGE)
		{
		...
		//Mutant Dolor Card
		if(CardNumSearch(849) && n_A_WeaponType == weapTyp_KATAR && n_A_Weapon_ATKplus >= 10)
			sizeMultiplier += 15;
	}
//Equips
...
{
	let targetPropertyMultipluer = StPlusCalc2( bon_DMG_ELE_NEUTRAL + Math.floor( n_B[en_ELEMENT] / 10 )) + StPlusEnchant( bon_DMG_ELE_NEUTRAL + Math.floor( n_B[en_ELEMENT] / 10 ) );
	// Card Bonuses
	if(not_use_card == 0)
		targetPropertyMultipluer += StPlusCard( bon_DMG_ELE_NEUTRAL + Math.floor( n_B[en_ELEMENT] / 10 ));
	
	if( SkillSearch( skill_HP_BASILICA ) && 
		((Math.floor( n_B[en_ELEMENT] / 10 ) == ele_UNDEAD) || (Math.floor( n_B[en_ELEMENT] / 10 ) == ele_DARK)))
		targetPropertyMultipluer +=  5 * SkillSearch( skill_HP_BASILICA );
	// "Thanatos' Dolor Hat"
	if( EquipNumSearch( 1637 ) && 
		((Math.floor( n_B[en_ELEMENT] / 10 ) == ele_DARK) || (Math.floor( n_B[en_ELEMENT] / 10 ) == ele_HOLY)))
	{
		if(n_A_HEAD_DEF_PLUS < 13)
			targetPropertyMultipluer += n_A_HEAD_DEF_PLUS / 2 ;
		else
			targetPropertyMultipluer += 6 ;
	}
	// Wood Goblin
	if((CardNumSearch(552) && n_A_BODY_DEF_PLUS >=9) && 
		((Math.floor( n_B[en_ELEMENT] / 10 ) == ele_WATER) || (Math.floor( n_B[en_ELEMENT] / 10 ) == ele_EARTH)))
		targetPropertyMultipluer += 5 ;
	//Armor of Sixtus (all)
	for(var i = 0 ; i < 6 ; i++)
		if( (EquipNumSearch( 2037 + i ) && i != 3) &&
			((Math.floor( n_B[en_ELEMENT] / 10 ) == ele_WIND) || (Math.floor( n_B[en_ELEMENT] / 10 ) == ele_EARTH)) &&
			(n_A_BODY_DEF_PLUS >= 11))
			targetPropertyMultipluer += 30;
	//Four Mirrors
	if((EquipNumSearch(2075) && n_A_Weapon_ATKplus >= 11) &&
		((Math.floor( n_B[en_ELEMENT] / 10 ) == ele__FIRE) || (Math.floor( n_B[en_ELEMENT] / 10 ) == ele_DARK)))
...
function getATKMultiplier()
{
	...
	if(not_use_card == 0)
		ATKMultiplier += StPlusCard( bon_PHY_ATK);
//Cards
	// Byorgue Card
	if( n_A_JobSearch2() === cls_ROG && CardNumSearch( 479 ) )
		ATKMultiplier += 10;
	//Champion Card
	if(CardNumSearch(583) && (SU_AGI >= 110))
		ATKMultiplier += 7;
...
//Equipments
	...
	if( EquipNumSearch( 1342 ) && n_A_HEAD_DEF_PLUS >= 9 )
		ATKMultiplier += 3;
	//Evil Marching Hat
	if(EquipNumSearch(1514) && (n_A_HEAD_DEF_PLUS >= 9))
		ATKMultiplier += 5;
	//Str Glove
	...
	if( EquipNumSearch( 2028 ) )
		ATKMultiplier += Math.floor(n_A_BODY_DEF_PLUS / 4);
	//Iro only ?
	//Kagero & Oboro Dual Dagger Set
	// if(EquipNumSearch(2084)) //Kagero & Oboro Dual Dagger Set
	...
	if(EquipNumSearch(2212))
		ATKMultiplier += n_A_BODY_DEF_PLUS;
	//Korean Judge Hat
...
//Shadows
	...
	if( EquipNumSearch( 1715 ) && ((n_A_SHADOW_WEAPON_DEF_PLUS + n_A_SHADOW_EARRING_DEF_PLUS + n_A_SHADOW_PENDANT_DEF_PLUS) >= 23))
		ATKMultiplier += 1;
	...
	if(n_B[en_BOSS] === 1)
	{
		...
		{//Lord of the Dead Helm
			if(n_A_HEAD_DEF_PLUS >= 5) 
				ATKMultiplier += n_A_HEAD_DEF_PLUS-5;
			if(CardNumSearch(31)) 
				ATKMultiplier += 5;
		}
	...
	if( n_B[en_BOSS] === 1 )
		ATKMultiplier += StPlusCalc2( bon_DMG_BOSS ) + StPlusCard( bon_DMG_BOSS ) + StPlusEnchant( bon_DMG_BOSS );
	else
		ATKMultiplier += StPlusCalc2( bon_DMG_NON_BOSS ) + StPlusCard( bon_DMG_NON_BOSS ) + StPlusEnchant( bon_DMG_NON_BOSS );
...
function getStatATK()
{
	...
	if( weaponType == weapTyp_BOW || weaponType == weapTyp_INSTRU ||
		weaponType == weapTyp_WHIP || weaponType == weapTyp_HANDGUN ||
		weaponType == weapTyp_RIFLE || weaponType == weapTyp_SHOTGUN ||
		weaponType == weapTyp_GATLING_GUN || weaponType == weapTyp_GRENADE_LAUNCHER)
		return getStatATKBonus(Math.floor((n_A_BaseLV/4)+ (n_A_STR / 5) + n_A_DEX + (n_A_LUK / 3))) * (element[n_B[en_ELEMENT]][ele_NEUTRAL] / 100);

	//StatusATK = (BaseLevel ÷ 4) + Str + (Dex ÷ 5) + (Luk ÷ 3)
	return getStatATKBonus(Math.floor((n_A_BaseLV/4)+ n_A_STR + (n_A_DEX / 5) + (n_A_LUK / 3))) * (element[n_B[en_ELEMENT]][ele_NEUTRAL] / 100);
}
function getStatATKBonus(batk)
{
	...
	if(SkillSearch(skill_SUM_BUNCH_OF_SHRIMP))
		batk += batk * SUM_BUNCH_OF_SHRIMP.skillFormula(SkillSearch(skill_SUM_BUNCH_OF_SHRIMP)) / 100;
...
function getWeaponATK(isCrit)
{
	...
	if(ATKOverUpgradeBonus)
		minATKOverUpgradeBonus = 1;


	
	weaponATK[0] = Math.floor((getBaseWeaponATK() - Math.floor(getWeaponATKVariance()) + Math.floor(getWeaponATKStatBonus()) + getWeaponATKRefinementBonus() + minATKOverUpgradeBonus + getWeaponATKHighUpgradeBonus()) * getSizePenalty());
	if(isCrit)
		weaponATK[0] = Math.floor((getBaseWeaponATK() + getWeaponATKVariance() + Math.floor(getWeaponATKStatBonus()) + getWeaponATKRefinementBonus() + minATKOverUpgradeBonus + getWeaponATKHighUpgradeBonus()) * getSizePenalty());
	weaponATK[2] = Math.floor((getBaseWeaponATK() + getWeaponATKVariance() + Math.floor(getWeaponATKStatBonus()) + getWeaponATKRefinementBonus() + ATKOverUpgradeBonus + getWeaponATKHighUpgradeBonus()) * getSizePenalty());
...
function getWeaponATK2(isCrit)
{
	...
	if(isCrit)
		weaponATK[0] = Math.floor((getBaseWeaponATK2() + Math.floor(getWeaponATKVariance2()) /*+ Math.floor(getWeaponATKStatBonus2())*/ + getWeaponATKRefinementBonus2() + minATKOverUpgradeBonus + getWeaponATKHighUpgradeBonus2()) * getSizePenalty2());
	weaponATK[2] = Math.floor((getBaseWeaponATK2() + Math.floor(getWeaponATKVariance2()) /*+ getWeaponATKStatBonus2() */+ getWeaponATKRefinementBonus2() + ATKOverUpgradeBonus + getWeaponATKHighUpgradeBonus()) * getSizePenalty2());
	weaponATK[1] = Math.floor((weaponATK[0] + weaponATK[2]) / 2);
	if(SkillSearch(skill_BS_POWER_MAXIMIZE))
		weaponATK[0] = weaponATK[1] = weaponATK[2] ;
	if(SkillSearch(GS_MAGICAL_BULLET)){
...
function getWeaponATKStatBonus()
{
	...
	if(isRanged)
		return getBaseWeaponATK() * n_A_DEX / 200;
...
function getWeaponATKStatBonus2()
{
	...
	// StatBonus = BaseWeaponDamage × Dex ÷ 200
	if(isRanged)
		return getBaseWeaponATK2() * n_A_DEX / 200;
...
function getWeaponATKOverUpgradeBonus()
{
	...
	{
		case 1:
			if(weaponRefineLevel > 7)
				return (weaponRefineLevel -7) * 3;
			break;
		case 2:
			if(weaponRefineLevel > 6)
				return (weaponRefineLevel -6) * 5;
			break;
		case 3:
			if(weaponRefineLevel > 5)
				return (weaponRefineLevel - 5) * 8;
			break;
		case 4:
			if(weaponRefineLevel > 4)
				return (weaponRefineLevel - 4) * 14;
			break;
		default:
...
function getExtraATK()
{
	...
	return EquipATK + ConsumableATK + AmmunitionATK + PseudoBuffATK + getWeaponATKBonus(0);
}
function getWeaponATKBonus(watk)
{
	...
	watk += RUN_FIGHTING_SPIRIT.skillFormula(SkillSearch(skill_RUN_FIGHTING_SPIRIT));
	...
	watk += SUR_GENTLE_TOUCH_CHANGE.skillFormula(SkillSearch(skill_SUR_GENTLE_TOUCH_CHANGE));
...
//Enchants
	if(EnchNumSearch( 841 ))//Special STR = 281
	{
		if(n_A_SHOULDER_DEF_PLUS >8)
		{
			equipmentAttack += 1;
	...
	if(EnchNumSearch( 842 ))//Special AGI = 282
	{
		if(n_A_SHOULDER_DEF_PLUS >8)
		{
			equipmentAttack += 1;
...
function getMasteryATK()
{
	...
	masteryATK += RAN_RANGER_MAIN.skillFormula(SkillSearch(skill_RAN_RANGER_MAIN));
	//element
	masteryATK += MEC_RESEARCH_FIRE_EARTH.skillFormula(SkillSearch(skill_MEC_RESEARCH_FIRE_EARTH));

	//NC_RESEARCHFE ???

	//weapon
	masteryATK += SW_SWORD_MASTERY.skillFormula(SkillSearch(skill_SW_SWORD_MASTERY));
	masteryATK += SW_TWO_HAND_SWORD_MASTERY.skillFormula(SkillSearch(skill_SW_TWO_HAND_SWORD_MASTERY));
	masteryATK += KN_SPEAR_MASTERY.skillFormula(SkillSearch(skill_KN_SPEAR_MASTERY));
	masteryATK += AS_KATAR_MASTERY.skillFormula(SkillSearch(skill_AS_KATAR_MASTERY));
	masteryATK += MO_IRON_FIST.skillFormula(SkillSearch(skill_MO_IRON_FIST));
	masteryATK += BA_MUSIC_LESSONS.skillFormula(SkillSearch(skill_BA_MUSIC_LESSONS));
	masteryATK += DA_DANCE_LESSONS.skillFormula(SkillSearch(skill_DA_DANCE_LESSONS));
	...
	masteryATK += MEC_AXE_TRAINING.skillFormula(SkillSearch(skill_MEC_AXE_TRAINING));
	masteryATK += GEN_SWORD_TRAINING.skillFormula(SkillSearch(skill_GEN_SWORD_TRAINING));
	//all weapons
	masteryATK += BS_WEAPONRY_RESEARCH.skillFormula(SkillSearch(skill_BS_WEAPONRY_RESEARCH));//listed as mastery in rathena
	masteryATK += MEC_MAGIC_GEAR_LICENSE.skillFormula(SkillSearch(skill_MEC_MAGIC_GEAR_LICENSE));
	//NV_BREAKTHROUGH <- extended novice skill

	//others
	if(SkillSearch(skill_SUR_RISING_DRAGON))
		masteryATK += SUR_RISING_DRAGON.skillFormula(SkillSearch(skill_SUR_RISING_DRAGON));
	else
		masteryATK += MO_SUMMON_SPIRIT_SPHERE.skillFormula(SkillSearch(skill_MO_SUMMON_SPIRIT_SPHERE));
	...
	masteryATK += BS_HILT_BINDING.skillFormula(SkillSearch(skill_BS_HILT_BINDING))
	...
	// }
	if(n_A_ActiveSkill == skill_NIN_THROW_HUUMA_SHURIKEN || n_A_ActiveSkill == skill_NIN_THROW_DAGGER || n_A_ActiveSkill == skill_NIN_THROW_KUNAI )
		masteryATK += NIN_DAGGER_THROWING_PRACTICE.skillFormula(SkillSearch(skill_NIN_DAGGER_THROWING_PRACTICE))
	...
	// 		break;
	// }
	if(n_A_ActiveSkill == skill_RAN_WARG_BITE || n_A_ActiveSkill == skill_RAN_WARG_DASH || n_A_ActiveSkill == skill_RAN_WARG_STRIKE)
		masteryATK += RAN_TOOTH_OF_WARG.skillFormula(SkillSearch(skill_RAN_TOOTH_OF_WARG));
	...
	// 	}
	masteryATK += REB_PLATINUM_ALTAR.skillFormula(SkillSearch( skill_REB_PLATINUM_ALTAR ));
...
function getBuffATK()
{
	...
	// if(sc->getSCE(SC_FIGHTINGSPIRIT))
	// 	watk += sc->getSCE(SC_FIGHTINGSPIRIT)->val1;
	buffATK += RUN_FIGHTING_SPIRIT.skillFormula(SkillSearch(skill_RUN_FIGHTING_SPIRIT));
	...
	if( SkillSearch( skill_SUM_MEOW_MEOW ) || summonerBuffs[ksMeowMeow])//not sure if misplaced
	{ // Meow Meow
		buffATK += 100;
	}
	...
	// Skills
	if( SkillSearch( skill_GS_LAST_STAND ) )//misplaced
	{ // LastStand
		buffATK += 100;
	}
	if( SkillSearch( skill_GS_GATLING_FEVER ) )//misplaced
	{ // Gatling Fever
	...
	if( SkillSearch( skill_ROY_BANDING ) )//not sure if misplaced
	{ // Banding ATK increase: [# of Royal Guard party members x (10 + 10 * Skill Level)]
		if(PATCH < 2)
			buffATK += ( 10 + 10 * SkillSearch( skill_ROY_BANDING ) ) * SkillSearch( skill_ROY_NUM_GUARDS );
	}

	if( performerBuffs[ksEnsemble] === ksBattleTheme && performerBuffs[ksEnsembleLevel] > 0 )//not sure if misplaced
	{ // Battle Theme
	...
	if( performerBuffs[ksChorus] === ksSaturdayNightFever &&
		 ...
		 performerBuffs[ksNumPerformers] >= 2 )//not sure if misplaced
...
function getRangedMultiplier()
{
	...
	// Calc Ranged bonuses
	let rangedMultiplier = StPlusCalc2( bon_DMG_RANGE) + StPlusEnchant( bon_DMG_RANGE );
	if(not_use_card == 0)
		rangedMultiplier += StPlusCard( bon_DMG_RANGE);
	
// Equipment

	if( (EquipNumSearch( 626 ) && n_A_Arrow == arrTyp_FIRE) ||// Burning Bow
		(EquipNumSearch( 627 ) && n_A_Arrow == arrTyp_CRYSTAL) ||// Freezing Bow
		(EquipNumSearch( 628 ) && n_A_Arrow == arrTyp_STONE)||// Earthen Bow
		(EquipNumSearch( 629 ) && n_A_Arrow == arrTyp_WIND) )// Gale Bow
		rangedMultiplier += 25;
	
	if( (EquipNumSearch( 2657 ) && n_A_Arrow == arrTyp_FIRE) ||// Elemental Tights + Burning Bow
		...
		(EquipNumSearch( 2660 ) && n_A_Arrow == arrTyp_WIND) )// Elemental Tights + Gale Bow
		rangedMultiplier += 20;

	if( (EquipNumSearch( 630 ) && n_A_Arrow == arrTyp_STEEL) ||// Orc Archer Bow
	...
	if( EquipNumSearch( 1408 ) )
		rangedMultiplier += 2 * n_A_BODY_DEF_PLUS;
	//Evil Marching Hat
	if(EquipNumSearch(1514) && (n_A_HEAD_DEF_PLUS >= 9) )
		rangedMultiplier += 5;
	...
	if( EquipNumSearch( 1464 ) )
	{ //Heroic Backpack
		if(n_A_SHOULDER_DEF_PLUS >= 7 && SU_DEX >= 90) { rangedMultiplier += 5; }
		if(n_A_SHOULDER_DEF_PLUS >= 9 && SU_DEX >= 90) { rangedMultiplier += 5; }
	}
	if( EquipNumSearch( 1583 ) )
	{ //Golden Angel Wing
		if(SU_DEX >= 90) { rangedMultiplier += 5; }
		if(n_A_SHOULDER_DEF_PLUS >= 9 && SU_DEX >= 90) { rangedMultiplier += 5; }
	}
	if( EquipNumSearch( 1584 ) )
	{ //Golden Angel Hairband
		if(SU_DEX >= 70) { rangedMultiplier += 2; }
		if(n_A_HEAD_DEF_PLUS >= 7 && SU_DEX >= 70) { rangedMultiplier += 3; }
	}
	if( EquipNumSearch( 1665 ) )
	{ //"Red Fox Ears(transformation mode)"
		if(n_A_HEAD_DEF_PLUS >= 6 && n_A_HEAD_DEF_PLUS <= 12) { rangedMultiplier += n_A_HEAD_DEF_PLUS - 5; }
		if(n_A_HEAD_DEF_PLUS >= 6 && n_A_HEAD_DEF_PLUS > 12) { rangedMultiplier += 7; }
	}
	//Fallen Angel Wing
	if(EquipNumSearch(1545))
		rangedMultiplier += Math.floor(SU_DEX/20);
	//"Dog Cap"
	if(EquipNumSearch(1702) && (n_A_HEAD_DEF_PLUS >= 11))
		rangedMultiplier += 3;
	...
	//Dex Boots
	if((EquipNumSearch(1950) || EquipNumSearch(1956) || EquipNumSearch(2201)) && (SU_DEX >= 120))
		rangedMultiplier += 5;
	...
	//Dark Rose [2]
	if(EquipNumSearch(2241) && (n_A_Weapon_ATKplus >= 7))
		rangedMultiplier += 15;
	// Dustfire [2]
	if( EquipNumSearch( 2243 ) && n_A_Arrow == bulTyp_ArmorPiercing )
		rangedMultiplier += 30;
	//Korean Judge Hat
	if(EquipNumSearch(2410) && (n_A_JobSearch() == cls_ARC || n_A_JobSearch() == cls_GUN ))
		rangedMultiplier += n_A_HEAD_DEF_PLUS;
...
//Shadow
	...
	if( EquipNumSearch(1842))
	{
		...
		if(n_A_SHADOW_WEAPON_DEF_PLUS >= 8){rangedMultiplier += 5;}
	}
	//Shadow Doram Battler Gloves
	if( EquipNumSearch(1995))
		rangedMultiplier += n_A_SHADOW_WEAPON_DEF_PLUS;
	//"Feathered Tricorn"
	if(EquipNumSearch(1860))
	{ 
		if(n_A_HEAD_DEF_PLUS >= 7){rangedMultiplier += 7;}
		if(n_A_HEAD_DEF_PLUS >= 11){rangedMultiplier += 5;}
	}
	//"Elaborate Yellow Foxtail Replica"
	if(EquipNumSearch(1881))
...
//Cards
	...
	if(CardNumSearch( 541 ))
		rangedMultiplier += Math.floor(SU_DEX/10);
	//Big Eggring Card
	if(CardNumSearch(589))
	{
		if(SU_DEX <=50)
			rangedMultiplier -= 1 * Math.floor(SU_DEX/10);
		else
			rangedMultiplier -= 5;
	}
	// Airship Raid Card
	...
	if(CardNumSearch(771))
	{
		if( n_A_WeaponType == weapTyp_HANDGUN || 
			...
			n_A_WeaponType == weapTyp_GRENADE_LAUNCHER)
		{
			rangedMultiplier += (5 * CardNumSearch(771)) + (n_A_Weapon_ATKplus * CardNumSearch(771));
			if(n_A_Weapon_ATKplus >= 10)
				rangedMultiplier += (5 * CardNumSearch(771));
		}
	}
	//Resentful Soldier Card
	if(CardNumSearch(818) && (n_A_WeaponType == weapTyp_BOW && n_A_Weapon_ATKplus >= 10))
...
//Enchants
	//Rune of Dexterity 1
	if(EnchNumSearch( 5163 ) && (n_A_BODY_DEF_PLUS >= 10))
		rangedMultiplier += 5;
	//Rune of Dexterity 2
	if(EnchNumSearch( 5164 ) && (n_A_BODY_DEF_PLUS >= 11))
		rangedMultiplier += 7;
	//Rune of Dexterity 3
	...
	{
		if(n_A_BODY_DEF_PLUS >= 7)
			rangedMultiplier += 2 * EnchNumSearch( 5245 );
		if(n_A_BODY_DEF_PLUS >= 9)
			rangedMultiplier += 2 * EnchNumSearch( 5245 );
	}
...
function getSoftDEF(isMonster)
{
	...
	if(isMonster)
		return Math.floor((n_B[en_VIT] + n_B[en_LEVEL])/2);
	...
	if(( SkillSearch (skill_AC_ANGELUS) || acolyteBuffs[ksAngelus] ))
		if( SkillSearch (skill_AC_ANGELUS) )
			...
			bonusB += 5 * acolyteBuffs[ksAngelus];
...
function getDamageMultiplier()
{
	...
	*/
	...
	if(n_A_ActiveSkill != PA_MARTYR_RECONING)
	{
		...
		if(SkillSearch(skill_BS_POWER_THRUST)==0)//doesn't stack with power thrust
			damageMutltiplier += MS_MAXIMUM_POWER_THUST.skillFormula(SkillSearch( skill_MS_MAXIMUM_POWER_THUST ));

		/// if(sc->getSCE(SC_MAXOVERTHRUST))
		/// 	skillratio += sc->getSCE(SC_MAXOVERTHRUST)->val2;
		damageMutltiplier += BS_POWER_THRUST.skillFormula(SkillSearch(skill_BS_POWER_THRUST));

		/// if(sc->getSCE(SC_BERSERK))
		/// 	skillratio += 200;
		if( SkillSearch( skill_LK_FRENZY ) )
			damageMutltiplier += 200;// is 200 on rathena ?

		/// if (sc && sc->getSCE(SC_TRUESIGHT))
		/// 	skillratio += 2 * sc->getSCE(SC_TRUESIGHT)->val1;
		damageMutltiplier += SN_FALCON_EYES.skillFormula(SkillSearch(skill_SN_FALCON_EYES));
		...
		if((n_A_ActiveSkill != skill_RUN_DRAGON_BREATH) && (n_A_ActiveSkill != skill_RUN_DRAGON_BREATH_WATER))
			damageMutltiplier += LK_SPEAR_DYNAMO.skillFormula(SkillSearch(skill_LK_SPEAR_DYNAMO));
	...
	{
		damageMutltiplier = ( n_A_BaseLV + n_A_STR + n_A_LUK + n_A_DEX ) / ( 12 - SkillSearch( skill_TKM_STELLAR_WRATH ) * 3 );
	}
	...
	{
		damageMutltiplier = ( n_A_BaseLV + n_A_LUK + n_A_DEX ) / ( 12 - SkillSearch( skill_TKM_SOLAR_WRATH ) * 3 );
	}
	else if( SkillSearch( skill_TKM_LUNAR_WRATH ) && n_B[en_SIZE] == 1 && n_B[en_HP] >= 5218 )
	{
		damageMutltiplier = ( n_A_BaseLV + n_A_LUK + n_A_DEX ) / ( 12 - SkillSearch( skill_TKM_LUNAR_WRATH ) * 3 );
	}
...
function getFinalDamageMultiplier(currentSkill)
{
	...
	if( n_A_WeaponType === weapTyp_KATAR && SkillSearch( skill_AX_ADVANCED_KATAR_MASTERY ) )
		FinalDamageMultiplier += AX_ADVANCED_KATAR_MASTERY.skillFormula(SkillSearch(skill_AX_ADVANCED_KATAR_MASTERY));

	if(SkillSearch(skill_GS_GUNSLINGER_PANIC))
		FinalDamageMultiplier -= 20;

	if((Skill[n_A_ActiveSkill].id == skill_MO_GUILLOTINE_FIST || Skill[n_A_ActiveSkill].id == skill_MO_MAX_GUILLOTINE_FIST) && SkillSearch( skill_SUR_RISING_DRAGON ))
...
function getCriticalMultiplier()
{
	...
	//bon_DMG_CRIT
	let criticalMultiplier = StPlusCalc2( bon_DMG_CRIT) + StPlusEnchant( bon_DMG_CRIT );
	if(not_use_card == 0)
		criticalMultiplier += StPlusCard( bon_DMG_CRIT);
		
//Equipment
	// Glorious Hunter Bow
	if ( EquipNumSearch( 1089 ) )
		criticalMultiplier += ( 2 * n_A_Weapon_ATKplus );
	...
	//Heroic Backpack
	if (EquipNumSearch( 1464 )){
		if ( SU_LUK >= 90 && n_A_SHOULDER_DEF_PLUS >= 7) { criticalMultiplier += 10; }
		if ( SU_LUK >= 90 && n_A_SHOULDER_DEF_PLUS >= 9) { criticalMultiplier += 5; }
	}
	//Golden Angel Wing
	if (EquipNumSearch( 1583 )){ 
		if ( SU_LUK >= 90 ) { criticalMultiplier += 5; }
		if ( SU_LUK >= 90 && n_A_SHOULDER_DEF_PLUS >= 9) { criticalMultiplier += 5; }
	}
	//Golden Angel Hairband
	if (EquipNumSearch( 1584 )){ 
		if ( SU_LUK >= 70 ) { criticalMultiplier += 2; }
		if ( SU_LUK >= 70 && n_A_HEAD_DEF_PLUS >= 7) { criticalMultiplier += 3; }
	}
	//Fallen Angel Wing
	if(EquipNumSearch(1545))
		criticalMultiplier += Math.floor(SU_LUK/20);
	//"Dog Cap"
	if(EquipNumSearch(1702) && n_A_HEAD_DEF_PLUS >= 11)
	...
	//"General's Helmet"
	if(EquipNumSearch(1942) &&(n_A_HEAD_DEF_PLUS >= 11))
	...
	//Gigant Blade 
	if( EquipNumSearch(2021) )
		criticalMultiplier += n_A_Weapon_ATKplus;
	...
	if(EquipNumSearch(2227) )  
		criticalMultiplier += n_A_HEAD_DEF_PLUS;
	// Fallen Warrior Manteau
	if(EquipNumSearch(2229)){
	...
	if(n_A_Equip[eq_ACCI] == 2449 && (n_A_card[card_loc_ACCI] == 653 || n_A_card[card_loc_ACCI] == 654 || n_A_card[card_loc_ACCI] == 655))
		criticalMultiplier += 5;
	//Demon God's Ring (Acc Slot 2)
	if(n_A_Equip[eq_ACCII] == 2449 && (n_A_card[card_loc_ACCII] == 653 || n_A_card[card_loc_ACCII] == 654 || n_A_card[card_loc_ACCII] == 655))
			criticalMultiplier += 5;

//Enchants
	//Rune of Luck 1
	if(EnchNumSearch( 5166 ) && (n_A_BODY_DEF_PLUS >= 10))
		criticalMultiplier += 5;
	//Rune of Luck 2
	if(EnchNumSearch( 5167 ) && (n_A_BODY_DEF_PLUS >= 11))
...
//Shadows
	...
	if( EquipNumSearch(1825)){
		if(n_A_SHADOW_SHIELD_DEF_PLUS >= 7){criticalMultiplier += 2;}
		if(n_A_SHADOW_SHIELD_DEF_PLUS >= 9){criticalMultiplier += 3;}
	}
...
//Cards
	...
	if(CardNumSearch(562))
		criticalMultiplier += Math.floor(SU_LUK/10) * 2;
	//Big Eggring Card
	if(CardNumSearch(589)){
		if(SU_LUK <=50)
			criticalMultiplier -= 2 * Math.floor(SU_LUK/10);
		else
			criticalMultiplier -= 10;
	}
	// Faceworm Queen Card
	if ( CardNumSearch( 635 ))
		criticalMultiplier += n_A_SHOES_DEF_PLUS;
	//Powerful Soldier Skeleton Card
	if(CardNumSearch(724) && (n_A_BaseLV >= 100))
		criticalMultiplier += 5;
	//Mutant Dolor Card
	if(CardNumSearch(849) && (n_A_WeaponType == weapTyp_KATAR))
		criticalMultiplier += n_A_Weapon_ATKplus * 2;
	//Ominous Assaulter Card
	if(CardNumSearch(893) && ((n_A_WeaponType == weapTyp_WHIP) || (n_A_WeaponType == weapTyp_INSTRUMENT) || (n_A_WeaponType == weapTyp_BOOK) || (n_A_WeaponType == weapTyp_SWORDII) || (n_A_WeaponType == weapTyp_SPEARII)))
...
function getSkillMultiplier(currentSkill)
{
	...
	if ( ( currentSkill == skill_AS_SONIC_BLOW ||
	...
	(currentSkill == skill_SUM_PICKY_PECK || currentSkill == skill_SUM_SCAR_OF_TAROU || 
	 currentSkill == skill_SUM_LUNATIC_CARROT_BEAT || currentSkill == skill_SUM_SPIRIT_OF_SAVAGE))
	{
	...
	if( (EquipNumSearch( 1723 ) && currentSkill==skill_CR_GRAND_CROSS) || // Shadow Crusader Armor
		...
		(EquipNumSearch( 1750 ) && currentSkill==skill_HU_LAND_MINE)) // Shadow Hunter Armor
	{ 
		testSkMod += n_A_SHADOW_BODY_DEF_PLUS * 5;
	}
	
	if( (EquipNumSearch( 1721 ) && currentSkill==skill_KN_PIERCE) || // Shadow Knight Boots
		...
		(EquipNumSearch( 1751 ) && currentSkill==skill_SN_FALCON_ASSAULT)) // Shadow Hunter Boots
	{ 
		testSkMod += n_A_SHADOW_SHOES_DEF_PLUS * 5;
	}
	
	if( (EquipNumSearch( 1810 ) && currentSkill==skill_ROY_EARTH_DRIVE) || // Shadow Royalguard Shield
		...
		(EquipNumSearch( 1814 ) && currentSkill==skill_SUR_SKY_NET_BLOW) ) // Shadow Sura Shield
		// (EquipNumSearch( 1817 ) && currentSkill==skill_WAR_EARTH_STRAIN) ) // Shadow Warlock Shield
	{ 
		testSkMod += n_A_SHADOW_SHIELD_DEF_PLUS * 5;
	}
	
	if( (EquipNumSearch( 1826 ) && currentSkill==skill_RUN_SONIC_WAVE) || // Shadow Runeknight Gloves
		...
		(EquipNumSearch( 1836 ) && currentSkill==skill_RAN_CLUSTER_BOMB) )		// Shadow Ranger Gloves
	{ 
		testSkMod += n_A_SHADOW_WEAPON_DEF_PLUS * 5;
	}
	
	if( (EquipNumSearch( 1860 ) && EquipNumSearch( 1193 ) && currentSkill==skill_RAN_ARROW_STORM) )
	{ // "Feathered Tricorn" + "Piece OF Angent Skin"
		testSkMod += Math.floor(n_A_SHOULDER_DEF_PLUS / 2) * 12;
	}
	if( (EquipNumSearch( 1860 ) && EquipNumSearch( 996 ) && currentSkill==skill_MIWA_SEVERE_RAINSTORM) )
	{ // "Feathered Tricorn" + "Leather of Tendrilion"
		testSkMod += Math.floor(n_A_SHOULDER_DEF_PLUS / 2) * 7;
	}
	if( (EquipNumSearch( 1860 ) && EquipNumSearch( 315 ) && currentSkill==skill_SHA_TRIANGLE_SHOT) )
	...
	if(currentSkill == skill_SUM_LUNATIC_CARROT_BEAT)
	{
		...
		{//"Shadow Doram Battler Shield"
			if(n_A_SHADOW_SHIELD_DEF_PLUS >= 7){testSkMod += 5;}
			if(n_A_SHADOW_SHIELD_DEF_PLUS >= 9){testSkMod += 5;}
		}
	...
	if(currentSkill == skill_NIN_FLAMING_PETALS)
	{
		...
		{//Wind Spear Petal Shadow Earring + Wind Spear Wind Spear Petal Shadow Pendant + Wind Spear Petal Shadow Shoes
			testSkMod += (n_A_SHADOW_EARRING_DEF_PLUS + n_A_SHADOW_PENDANT_DEF_PLUS + n_A_SHADOW_SHOES_DEF_PLUS)/2;
		}
	}
	if(currentSkill == skill_NIN_FREEZING_SPEAR)
	{
	...
	if(currentSkill == skill_MEC_AXE_TORNADO && EquipNumSearch(2085)) //"Dog Cap + Tornado Axe"
	{
		testSkMod += 15 * Math.floor(n_A_Weapon_ATKplus / 2);
	}
	if(currentSkill == skill_SUR_TIGER_CANNON && EquipNumSearch(2074)) //Claws of the Bifrost
	{
		testSkMod += 4 * Math.floor(n_A_Weapon_ATKplus / 3);
	}
	if(((currentSkill == skill_RUN_DRAGON_BREATH || currentSkill == skill_RUN_DRAGON_BREATH_WATER) && EquipNumSearch(2051))) //Dragon Slayer (Ancient Weapon)
	{
		testSkMod += 5 * Math.floor(n_A_Weapon_ATKplus / 3);
	}
	if((currentSkill == skill_RUN_SONIC_WAVE && EquipNumSearch(2050)) ||//Runic Katana
	   ...
	   (currentSkill == skill_GEN_CART_TORNADO && EquipNumSearch(2070)))//Slate Sword
	{
		testSkMod += 10 * Math.floor(n_A_Weapon_ATKplus / 3);
	}	
	if((currentSkill == skill_RAN_ARROW_STORM && EquipNumSearch(2061))  || //Empyrean
	   ...
	   (currentSkill == skill_SOR_EARTH_GRAVE && EquipNumSearch(2065))   )  //Shadow Eater
	{
		testSkMod += 12 * Math.floor(n_A_Weapon_ATKplus / 3);
	}
	if(currentSkill == skill_ABI_DUPLE_LIGHT && EquipNumSearch(2071)) //All-Holy Book
	{
		testSkMod += 25 * Math.floor(n_A_Weapon_ATKplus / 3);
	}
	
	if((currentSkill == skill_MIWA_SEVERE_RAINSTORM  && EquipNumSearch(2054) && n_A_Weapon_ATKplus >= 9) ||  //"Bow of Narcissus"
	  ((currentSkill == skill_MEC_VULCAN_ARM || currentSkill == skill_MEC_ARM_CANNON ) && EquipNumSearch(2068) && n_A_Weapon_ATKplus >= 9) ||  //Big Badaboom
	   (currentSkill == skill_SUR_RAMPAGE_BLASTER && EquipNumSearch(2073) && n_A_Weapon_ATKplus >= 9) )  //Iron Claw
	{
		testSkMod += 10;
	}
	if(((currentSkill == skill_ROY_SPEAR_CANNON || currentSkill == skill_ROY_VANISHING_POINT) && EquipNumSearch(2052) && n_A_Weapon_ATKplus >= 7) || //Trident of Undine
	    (currentSkill == skill_GEN_CART_CANNON && EquipNumSearch(2069) && n_A_Weapon_ATKplus >= 9) || //Sword of Blue Fire
	    (currentSkill == skill_SUR_TIGER_CANNON && EquipNumSearch(2074) && n_A_Weapon_ATKplus >= 9)|| //Claws of the Bifrost
	    (currentSkill == skill_REB_DRAGON_TAIL && EquipNumSearch(2081) && n_A_Weapon_ATKplus >= 7) ||//Big Game Trophy
	    (currentSkill == skill_REB_ROUND_TRIP && EquipNumSearch(2082) && n_A_Weapon_ATKplus >= 7)  ) //Guttling Gun
	{
		testSkMod += 15;
		if((currentSkill == skill_REB_DRAGON_TAIL && EquipNumSearch(2081) && n_A_Weapon_ATKplus >= 11) ||//Big Game Trophy
		   (currentSkill == skill_REB_ROUND_TRIP && EquipNumSearch(2082) && n_A_Weapon_ATKplus >= 11)  ) //Guttling Gun
			testSkMod += 15;
	}
	if((currentSkill == skill_GLT_ROLLING_CUTTER && EquipNumSearch(2058) && n_A_Weapon_ATKplus >= 9) || //Steel Flower
	   ((currentSkill == skill_ROY_SPEAR_CANNON || currentSkill == skill_ROY_VANISHING_POINT) && EquipNumSearch(2052) && n_A_Weapon_ATKplus >= 11) ||//Trident of Undine
	   (currentSkill == skill_GEN_CART_TORNADO && EquipNumSearch(2070) && n_A_Weapon_ATKplus >= 9)   ) //Slate Sword
	{
		testSkMod += 20;
	}
	if(currentSkill == skill_KAG_CROSS_STRIKE && EquipNumSearch(2084) && (n_A_Weapon_ATKplus + n_A_Weapon2_ATKplus) >= 14) 
	{//Kagero & Oboro Dual Dagger Set
		testSkMod += 25;
		if(currentSkill == skill_KAG_CROSS_STRIKE && EquipNumSearch(2084) && (n_A_Weapon_ATKplus + n_A_Weapon2_ATKplus) >= 20)
		{//Kagero & Oboro Dual Dagger Set
			testSkMod += 25;
			
		}
	}
	if((currentSkill == skill_ABI_DUPLE_LIGHT && EquipNumSearch(2071) && n_A_Weapon_ATKplus >= 11) ||//All-Holy Book
	   (currentSkill == skill_KAG_SWIRLING_PETAL && EquipNumSearch(2075) && n_A_Weapon_ATKplus >= 7))//Four Mirrors
	{
		testSkMod += 30;
	}
	if(currentSkill == skill_RAN_AIMED_BOLT && EquipNumSearch(2062) && n_A_Weapon_ATKplus >= 9) //Scarlet Dragon's Bow
	{
		testSkMod += 35;
	}
	
	if(currentSkill == skill_MEC_AXE_BOOMERANG && EquipNumSearch(2067) && n_A_Weapon_ATKplus >= 9) //Avenger
	{
		testSkMod += 40;
	}
	if(currentSkill == skill_MA_COLD_BOLT || currentSkill == skill_MA_FIRE_BOLT || currentSkill == skill_MA_LIGHTNING_BOLT )
	{
	...
	if(currentSkill == skill_REB_ANTI_MATERIAL_BLAST || currentSkill == skill_REB_GODS_HAMMER )
	{
		if(CardNumSearch( 690 ))
		{ //Captain Ferlock Card
			if(n_A_Weapon_ATKplus >= 10)
				testSkMod += 30 * CardNumSearch( 672 );
		}
	}
	if(currentSkill == skill_REB_ANTI_MATERIAL_BLAST || currentSkill == skill_REB_MASS_SPIRAL )
	{
	...
	if((EquipNumSearch(2279) && currentSkill == skill_RUN_DRAGON_BREATH) 	|| // Wyrmeater's Shadow Pendant
	   ...
	   (EquipNumSearch(2290) && currentSkill == skill_GEN_CART_CANNON) 		)  // Boscard's Shadow Pendant
	{
		testSkMod += 2 * Math.floor(n_A_SHADOW_PENDANT_DEF_PLUS/2);
	}
	
	if((EquipNumSearch(2287) && currentSkill == skill_WI_METEOR_STORM))// Dordaleon's Shadow Pendant
		testSkMod += 3 * Math.floor(n_A_SHADOW_PENDANT_DEF_PLUS/2);
		
	if((EquipNumSearch(2294) && currentSkill == skill_PR_MAGNUS_EXORCISMUS) 	|| // Exorcist Shadow Set
	   ...
	   (EquipNumSearch(2301) && currentSkill == skill_CG_ARROW_VULCAN) 		)  // Garmia's  Shadow Set
		testSkMod += n_A_SHADOW_WEAPON_DEF_PLUS + n_A_SHADOW_EARRING_DEF_PLUS + n_A_SHADOW_PENDANT_DEF_PLUS;
		
	if((EquipNumSearch(2297) && currentSkill == skill_MEC_ARM_CANNON) 		|| // Talos' Shadow Set
	   (EquipNumSearch(2302) && currentSkill == skill_GEN_CART_CANNON) 		)  // Boscard's Shadow Set
		testSkMod += Math.floor((n_A_SHADOW_WEAPON_DEF_PLUS + n_A_SHADOW_EARRING_DEF_PLUS + n_A_SHADOW_PENDANT_DEF_PLUS)/2);
	
	if( EquipNumSearch(2304) && currentSkill == skill_SUR_TIGER_CANNON )  //Sura set
	{
		// testSkMod += Math.floor((n_A_SHADOW_WEAPON_DEF_PLUS + n_A_SHADOW_SHIELD_DEF_PLUS));
	}
	if( EquipNumSearch(2303) && currentSkill == skill_RUN_DRAGON_BREATH)  //Rune Knight Set
	{
		testSkMod += n_A_SHADOW_WEAPON_DEF_PLUS + n_A_SHADOW_SHIELD_DEF_PLUS;
	}
	
	if( EquipNumSearch(2316) && currentSkill == skill_REB_FIRE_DANCE)  //Rune Knight Set
	{//Dog Cap + Dark Rose
		testSkMod += 6 * Math.floor(n_A_Weapon_ATKplus / 2);
	}
	
	if( EquipNumSearch(2587) && currentSkill == skill_REB_FIRE_DANCE)  //Rune Knight Set
	...
	if(currentSkill == skill_MEC_POWER_SWING || currentSkill == skill_MEC_AXE_TORNADO)
	{
	...
	if(currentSkill == skill_ROY_SHIELD_PRESS)
	{
		...
		{//Imperial Ring + Imperial Guard
			if(n_A_LEFT_DEF_PLUS >= 5)
				testSkMod += 8 * (n_A_LEFT_DEF_PLUS - 5);
		}
	}
	if(EquipNumSearch(2460) && n_A_HEAD_DEF_PLUS >= 9)
	{//Magician's Night Cap
		if(currentSkill == skill_MA_FIRE_BOLT || currentSkill == skill_MA_COLD_BOLT || currentSkill == skill_MA_LIGHTNING_BOLT )
		{
	  ...
	  currentSkill == skill_ABI_JUDEX  ) 
	{
		...
		if ( currentSkill === skill_MA_NAPALM_BEAT ||
	...
	if ( EquipNumSearch( 2662 ) && n_A_Arrow == arrTyp_FIRE && (currentSkill === skill_RAN_ARROW_STORM || currentSkill === skill_MIWA_SEVERE_RAINSTORM))
	{ // Elemental Tights + Burning Bow
		testSkMod += n_A_SHOULDER_DEF_PLUS * 5;
	}
	else if ( EquipNumSearch( 2663 ) && n_A_Arrow == arrTyp_CRYSTAL && (currentSkill === skill_RAN_ARROW_STORM || currentSkill === skill_MIWA_SEVERE_RAINSTORM))
	{ // Elemental Tights + Freezing Bow
		testSkMod += n_A_SHOULDER_DEF_PLUS * 5;
	}
	else if ( EquipNumSearch( 2664 ) && n_A_Arrow == arrTyp_STONE && (currentSkill === skill_RAN_ARROW_STORM || currentSkill === skill_MIWA_SEVERE_RAINSTORM))
	{ // Elemental Tights + Earthen Bow
		testSkMod += n_A_SHOULDER_DEF_PLUS * 5;
	}
	else if ( EquipNumSearch( 2665 ) && n_A_Arrow == arrTyp_WIND && (currentSkill === skill_RAN_ARROW_STORM || currentSkill === skill_MIWA_SEVERE_RAINSTORM))
