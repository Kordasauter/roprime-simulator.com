...
function clashingATKFormula(weight) {
  ...
  tempAttack[0] =
    (tempWeaponAttack - varianceAttack + minOverrefineAttack) *
    ((tempsizemod + 100) / 100) *
    (1 + (5 + SkillSearch(skill_LK_SPEAR_DYNAMO) * 2) / 100);
  tempAttack[2] =
    (tempWeaponAttack + varianceAttack + overrefineAttack) *
    ((tempsizemod + 100) / 100) *
    (1 + (5 + SkillSearch(skill_LK_SPEAR_DYNAMO) * 2) / 100);
  ...
  for (var i = 0; i < 3; i++) {
    ...
    tempAttack[i] +=
      (((weight + equipmentAttack + strengthBonusAttack) *
        element[n_B[en_ELEMENT]][n_A_Weapon_element]) /
        100) *
      0.7;
    // multiply race mod
    ...
    tempAttack[i] +=
      (statusAttack * 2 * 0.9 * element[n_B[en_ELEMENT]][ele_NEUTRAL]) / 100;
...
function CalcSkillDamageType() {
  ...
  if (n_A_ActiveSkill === skill_GS_DESPERADO) damageType = kDmgTypeMelee;
  if (n_A_ActiveSkill === skill_SUR_GATE_OF_HELL) {
    // Gate of Hell
    if (n_A_ActiveSkillLV > 4) damageType = kDmgTypeRanged;
    else damageType = kDmgTypeMelee;
  }
}
...
function CalcSkillDamage_old() {
  ...
  if (
    ...
    if (n_Nitou) {
      ...
      for (var i = 0; i < 3; i++) {
        lefthandAttack[i] = ApplyEnemyDefense(
          lefthandAttack[i] * w_SkillMod,
          i,
          0
        );
      ...
      if (SkillSearch(skill_MO_RAGING_TRIFECTA_BLOW)) {
        ...
        for (var i = 0; i <= 2; i++) {
          trifectaDamage[i] = CalcFinalDamage(
            n_A_DMG[i] * trifectaBlowSkillMod,
            i
          );
        ...
        str_bSUB +=
          trifectaDamage[0] + "~" + trifectaDamage[2] + " (30 % Chance)<BR>";
        trifectaBlowDamage = 0;
      ...
      if (SkillSearch(skill_ABI_DUPLE_LIGHT)) {
        ...
        str_bSUB +=
          ...
          DLDamage[2] +
          " (" +
          (10 + SkillSearch(skill_ABI_DUPLE_LIGHT) * 2) +
          "% Chance)<BR>";
      ...
      if (SkillSearch(skill_RUN_GIANT_GROWTH)) {
        ...
        str_bSUB +=
          trifectaDamage[0] + "~" + trifectaDamage[2] + " (15% Chance)<br/>";
        trifectaBlowDamage = 0;
      ...
      if (SkillSearch(skill_RUN_ENCHANT_BLADE)) {
        for (var i = 0; i < 3; i++) {
          n_A_DMG[i] +=
            (SkillSearch(skill_RUN_ENCHANT_BLADE) * 20 + 100) *
              (n_A_BaseLV / 150.0) +
            n_A_INT;
        }
      }
      ...
      if (n_A_WeaponType === weapTyp_KATAR) {
        for (var i = 0; i < 3; i++) {
          w_KATARU[i] = Math.floor(
            w_DMG[i] * (0.01 + SkillSearch(skill_TH_DOUBLE_ATTACK) * 0.02)
          );
        }
        katarOffhandDamage = Math.floor(
          w_DMG[1] * (0.01 + SkillSearch(skill_TH_DOUBLE_ATTACK) * 0.02)
        );
      }
    ...
    if (w998G >= 100) n_Min_DMG = n_A_CriATK[0];
    if (w998G > 0) n_Max_DMG = n_A_CriATK[2];
  ...
  if (n_A_ActiveSkill == w_ActS[iw]) {
    ...
    if (n_A_ActiveSkill === skill_SW_BASH) {
      ...
      if (n_A_WeaponType === weapTyp_SPEARII) {
        //prime
      ...
      variableCastTime *= 0.0;
      n_Delay[ksDelayGlobal] = 1.0;
    } else if (n_A_ActiveSkill == skill_MO_RAGING_TRIFECTA_BLOW) {
      //prime
      wActiveHitNum = 3;
      //var trifectaBlowSkillMod = 1.0 + trifectaBlowLevel * 0.2;
      w_SkillMod = 1 + n_A_ActiveSkillLV * 0.2;
      ...
      n_A_ActiveSkill == skill_BA_MELODY_STRIKE ||
      n_A_ActiveSkill == skill_DA_SLINGING_ARROW
    ) {
      ...
      if (EquipNumSearch(2277)) {
        ...
        n_Delay[ksDelayCooldown] -=
          0.1 * Math.floor(n_A_SHADOW_EARRING_DEF_PLUS / 3);
      }
      ...
      n_A_ActiveSkill == skill_HEAT ||
      n_A_ActiveSkill == skill_HEAT_WALL
    ) {
      fixedCastTime *= 0.0;
      variableCastTime *= 0.0;
      ...
      if (n_A_ActiveSkill == skill_TK_COUNTER_KICK) wActiveHitNum = 3;
      ...
      else w_SkillMod += n_A_BaseLV * 0.04 - 1;
      ...
      // 50%-250% damage
      w_SkillMod =
        ((n_A_ActiveSkillLV / 2.0 +
          SkillSearch(skill_KN_SPEAR_MASTERY) / 10.0) *
          n_A_BaseLV) /
        150.0;
      ...
      myInnerHtml(
        "CRIATK",
        '<font color="#0000FF"><b>' + damageOut + "</b></font>",
        0
      );
      ...
      myInnerHtml(
        "CRInum",
        '<font color="#FF0000"><b>' + damageIn + "</b></font>",
        0
      );
      ...
      w_SkillMod =
        n_A_WeaponLV * (n_A_Weapon_ATKplus + 6) +
        weaponAtk / 100.0 +
        weaponWeight / 100.0;
      ...
      w_SkillMod =
        SkillSearch(skill_KN_SPEAR_MASTERY) + Math.floor(n_A_STR / 8);
      ...
      if (PATCH < 2) {
        ...
        // 680%-1400% damage
        w_SkillMod =
          n_A_ActiveSkillLV * 0.8 + 6 + (1000 - weaponWeight) / 100.0;
        w_SkillMod *= n_A_BaseLV / 100;
      ...
      if (
        EquipNumSearch(2084) &&
        n_A_Weapon_ATKplus + n_A_Weapon2_ATKplus >= 16
      ) {
      ...
      if (PATCH <= 1) {
        ...
        w_SkillMod =
          (SkillSearch(skill_GEN_CART_REMODELING) * 50 * (n_A_INT / 40) +
            n_A_ActiveSkillLV * 60) /
          100;
      } else if (PATCH == 2) {
        // formula:[(Skill Level * 0.6 ) + {((Skill Level x 20) x Cart_Remodeling_Lv) + (INT x 2)}] x (BaseLv ÷ 100)
        w_SkillMod =
          n_A_ActiveSkillLV * 2.5 +
          n_A_ActiveSkillLV * 0.2 * SkillSearch(skill_GEN_CART_REMODELING) +
          n_A_INT * 0.02;
        w_SkillMod *= n_A_BaseLV / 100.0;
      }
      ...
      if (afterDragonCombo) {
        w_SkillMod =
          ((n_A_ActiveSkillLV * 1.0 + n_A_AGI / 100.0 + 1.5) * n_A_BaseLV) /
          100.0;
      } else {
        w_SkillMod =
          (n_A_ActiveSkillLV * 0.8 + n_A_AGI / 100.0) * (n_A_BaseLV / 100.0);
      }
      ...
      if (visible) {
        w_SkillMod =
          n_A_ActiveSkillLV * 0.5 * (n_A_BaseLV / 100.0) +
          (n_A_INT * 2) / 100.0;
      } else {
        w_SkillMod =
          n_A_ActiveSkillLV * 1.5 * (n_A_BaseLV / 100.0) +
          (n_A_INT * 3) / 100.0;
      }
      ...
      if (SkillSearch(skill_MO_FURY)) {
        w_SkillMod =
          ((SkillSearch(skill_MO_FURY) * 0.2 + n_A_ActiveSkillLV * 0.2) *
            numSpheres *
            n_A_BaseLV) /
          120.0;
      } else {
        w_SkillMod =
          (n_A_ActiveSkillLV * 0.2 * numSpheres * n_A_BaseLV) / 100.0;
      }
      ...
      if (knockback) {
        w_SkillMod +=
          n_A_ActiveSkillLV * 1.5 /*+ ( 10 * 1 )*/ +
          (((n_B[en_LEVEL] * 5) / 100.0) * n_A_BaseLV) / 150.0;
      }
      ...
      w_SkillMod =
        ((n_A_ActiveSkillLV * 1.0 + n_A_DEX / 100.0) * n_A_BaseLV) / 100.0;
      ...
      w_SkillMod =
        ((n_A_ActiveSkillLV * 0.5 + (n_A_STR * n_A_ActiveSkillLV) / 100.0) *
          n_A_BaseLV) /
        100.0;
      ...
      if (PATCH < 2) {
        ...
        w_SkillMod =
          ((n_A_ActiveSkillLV * 0.5 + bashLevel * 0.3) * n_A_BaseLV) / 100.0;
      } else if (PATCH == 2) {
        // ATK [{(Skill Level x 80) + (Caster s learned Bash Level x 50)} x Caster s Base Level / 100] %
        w_SkillMod =
          ((n_A_ActiveSkillLV * 0.8 + bashLevel * 0.5) * n_A_BaseLV) / 100.0;
      }
      if (EquipNumSearch(1269)) {
        // Imperial Spear gives 20% more damage
        var spearBonusDamage = 1.2;
      ...
      w_SkillMod =
        ((n_A_ActiveSkillLV + (n_A_AGI * 5) / 100.0) * n_A_BaseLV) / 100.0;
      ...
      w_SkillMod =
        n_A_ActiveSkillLV +
        (n_A_JobLV * 10) / 100.0 +
        (weaponWeight * weaponLevel * (n_A_BaseLV / 100.0)) / 100.0;

      // Crit is a guarantee, calc it.
      wCriTyuu = 1;
      for (var i = 0; i < 3; i++) {
        n_A_CriATK[i] = Math.floor(
          CalcFinalDamage(n_A_CriATK[i], 10) * w_SkillMod
        );
      }
      wCriTyuu = 0;
      ...
      if (PATCH < 2) {
        ...
        w_SkillMod =
          ((n_A_ActiveSkillLV * 2 + SkillSearch(skill_CR_SPEAR_QUICKEN) * 0.5) *
            n_A_BaseLV) /
          100.0;
        ...
        if (numHits >= 2) {
          ...
          w_SkillMod +=
            ((n_A_ActiveSkillLV + (n_A_STR + n_A_DEX) / 100.0) * n_A_BaseLV) /
            100.0;
        }
        if (numHits >= 3) {
      ...
      w_SkillMod =
        ((n_A_ActiveSkillLV * 1.2 + SkillSearch(skill_ROY_OVERBRAND) * 0.8) *
          n_A_BaseLV) /
        100.0;
      ...
      if (PATCH < 2) {
        ...
        w_SkillMod =
          ((((n_A_ActiveSkillLV + 1) * shieldWeight) / 100.0) * n_A_BaseLV) /
          100.0;
      } else if (PATCH == 2) {
      ...
      if (n_A_WeaponType == weapTyp_AXE || n_A_WeaponType == weapTyp_AXEII) {
        w_SkillMod =
          ((n_A_ActiveSkillLV * 0.5 + 2.5 + axeWeight / 100.0) * n_A_BaseLV) /
          100.0;
      }
      ...
      damageType = kDmgTypeRanged;
      w_SkillMod =
        ((n_A_ActiveSkillLV + 2.0 + n_A_DEX / 100.0) * n_A_BaseLV) / 120.0;
      ...
      w_SkillMod =
        ((n_A_ActiveSkillLV + 3.0 + n_A_STR / 100.0) * n_A_BaseLV) / 100.0;
      ...
      w_SkillMod =
        ((n_A_ActiveSkillLV * 0.7 + n_A_DEX / 100.0) * n_A_BaseLV) / 120.0;
      ...
      if (PATCH < 2) {
        ...
        w_SkillMod =
          ((n_A_ActiveSkillLV * sizeModifier + 3.0) * n_A_BaseLV) / 120.0;
      }
      if (PATCH == 2) {
      ...
      n_A_ActiveSkill == skill_CR_SHIELD_BOOMERANG_SL
    ) {
      n_PerHIT_DMG = 0;
      damageType = kDmgTypeRanged;
      n_A_Weapon_element = ele_NEUTRAL;
      ...
      w_SkillMod +=
        (n_A_LEFT_DEF_PLUS * 5 + ItemOBJ[n_A_Equip[eq_SHIELD]][itm_WEIGHT]) /
        100;
      ...
      w_SkillMod +=
        (n_A_LEFT_DEF_PLUS * 4 + ItemOBJ[n_A_Equip[eq_SHIELD]][itm_WEIGHT]) /
        100;
      w_SkillMod *= n_A_BaseLV / 100;
      ...
      if (EquipNumSearch(2414)) {
        //Rebellion's Scarf
        n_Delay[ksDelayCooldown] -=
          0.1 * SkillSearch(skill_REB_SHATTERING_STORM);
      }
      ...
      if (EquipNumSearch(2242)) {
        //Finisher [2]
        if (n_A_Weapon_ATKplus >= 9) n_Delay[ksDelayCooldown] -= 1.0;
      }
      ...
      if (EquipNumSearch(2542)) {
        //HR-S55-OS
        if (n_A_Weapon_ATKplus >= 11) n_Delay[ksDelayCooldown] -= 5.0;
      }
      ...
      n_Delay[ksDelayGlobal] = 2.0;
      n_Delay[ksDelayCooldown] = 5.0;
      if (EquipNumSearch(2081) && n_A_Weapon_ATKplus >= 11)
        //Big Game Trophy
        n_Delay[ksDelayCooldown] -= 1.0;
      ...
      if (n_A_ActiveSkillLV < 5) variableCastTime *= 1.0;
      else variableCastTime *= 0.0;
    } else if (n_A_ActiveSkill == skill_GEN_CART_TORNADO) {
      let Curr_Cart_Weight = eval(document.calcForm.SkillSubNum.value);
      if (isNaN(Curr_Cart_Weight)) Curr_Cart_Weight = 0;
      if (Curr_Cart_Weight > 8000) Curr_Cart_Weight = 8000;
      if (Curr_Cart_Weight < 0) Curr_Cart_Weight = 0;
      if (PATCH == 0) {
        //[(n_A_ActiveSkillLV x 50) + (Cart_Remodeling_Lv × 50) + {Curr_Cart_Weight ÷ (150 − Base_STR)}]%
        w_SkillMod =
          n_A_ActiveSkillLV * 0.5 +
          SkillSearch(skill_GEN_CART_REMODELING) * 0.5 +
          Curr_Cart_Weight / (150 - SU_STR) / 100;
    ...
    for (var i = 0; i < 3; i++) {
      ...
      } else w_DMG[i] = CalcFinalDamage(n_A_DMG[i], i);
      ...
      if (wActiveHitNum > 1) {
        InnStr[i] +=
          " (" + w_DMG[i] / wActiveHitNum + " x " + wActiveHitNum + "Hit)";
      }
    }
    w_MagiclBulet = 1;
    w_DMG[1] =
      (w_DMG[1] * w_HIT + ApplyDamageModifiers(0) * (100 - w_HIT)) / 100;
  ...
  // Physical normal attacks with subhits ----------------------------
    ...
    n_A_ActiveSkill === skill_AR_DOUBLE_STRAFE ||
    ...
    if (n_A_ActiveSkill === skill_AR_DOUBLE_STRAFE) {
      ...
      n_A_ActiveSkill === skill_MEC_AXE_TORNADO &&
      (n_A_WeaponType == weapTyp_AXE || n_A_WeaponType == weapTyp_AXEII)
    ) {
      // show HP drain
      myInnerHtml(
        "CRInumname",
        '<font color="#FF0000"><b>Health Drain</b></font>',
        0
      );
      myInnerHtml(
        "CRInum",
        '<font color="#FF0000"><b>' + 20 * n_A_ActiveSkillLV + "</b></font>",
        0
      );
      ...
      w_SkillMod =
        ((n_A_ActiveSkillLV + 2 + n_A_VIT / 100.0) * n_A_BaseLV) / 100.0;
      ...
      if (EquipNumSearch(2062) && n_A_Weapon_ATKplus >= 11)
      ...
      if (PATCH < 2) {
        ...
        w_SkillMod =
          ((n_A_ActiveSkillLV * 1.2 + numRoyalGuards * 2.0) * n_A_BaseLV) /
          100.0;
      ...
      if (PATCH < 2) {
        w_SkillMod =
          ((3.0 + (n_A_ActiveSkillLV - 1) * (n_A_AGI / 200)) * n_A_BaseLV) /
          120 /
          3;
        ...
        w_SkillMod =
          (2.3 * n_A_ActiveSkillLV * ((n_A_AGI * 3) / 100) * n_A_BaseLV) /
          100 /
          3;
        variableCastTime *= 0.0;
        n_Delay[ksDelayGlobal] = 0.35;
    ...
    for (var i = 0; i < 3; i++) {
      ...
      if (monsterDebuffs[status_en_LEXA] == 0 || wLAch == 0) {
        InnStr[i] +=
          Math.floor(w_DMG[i] * w_TotalHits) +
          " ( " +
          w_DMG[i] +
          SubName[8][Language] +
          w_TotalHits +
    ...
    if (w_TotalHits == 0 && n_A_ActiveSkill == skill_MO_THROW_SPIRIT_SPHERES) {
      InnStr[0] =
        "<font color=Red><b>No spheres available</b></font><br/>" + InnStr[0];
  ...
  // Physical but special formula skills --------------------------
    ...
    n_A_ActiveSkill === skill_HU_BLITZ_BEAT ||
    n_A_ActiveSkill === skill_SN_FALCON_ASSAULT
  ) {
    n_PerHIT_DMG = 0;
    n_A_Weapon_element = ele_NEUTRAL;
    damageType = kDmgTypeRanged;
    ...
    if (n_A_ActiveSkill == skill_SN_FALCON_ASSAULT) {
      ...
      wBT *=
        SkillSearch(skill_HU_STEEL_CROW) / 20 +
        n_A_ActiveSkillLV +
        n_A_BaseLV / 50; //prime
    ...
    if (n_A_ActiveSkill === skill_RAN_WARG_DASH) {
      ...
      for (i = 0; i < 3; i++) {
        TMPATK[i] =
          TMPATK[i] *
          (6 + n_A_ActiveSkillLV * 2 + 1 * (n_A_ActiveSkillLV == 5));
    ...
    wINV = Math.floor(
      (ApplyDamageModifiers(0) * element[n_B[en_ELEMENT]][ele_POISON]) / 100
    );
    n_PerHIT_DMG = wINV;
    ...
    for (var i = 0; i < 3; i++) {
      w_DMG[i] = CalcFinalDamage(n_A_DMG[i], i);
      w_DMG[i] = Math.floor(
        (w_DMG[i] * element[n_B[en_ELEMENT]][ele_POISON]) / 100
      );
      Last_DMG_A[i] = Last_DMG_B[i] = w_DMG[i];
      InnStr[i] += Last_DMG_A[i];
    }
    ...
    for (var i = 0; i < 3; i++) {
      physicalDamage[i] = ApplyEnemyDefense(
        n_A_DMG[i] * (((300 + 50 * n_A_ActiveSkillLV) * w_SkillMod) / 100),
        i,
        0
      );
    }
    ...
    str_PerHIT_DMG =
      minDamage + magicDamage[0] + "~" + (minDamage + magicDamage[2]);
    if (n_B[en_BOSS] === 5) {
      // plant monsters
      for (var i = 0; i < 3; i++) {
    ...
    myInnerHtml(
      "CRInumname",
      '<Font color="#FF0000">Damage Backlash</Font>',
      0
    );
    ...
    for (var i = 0; i < 3; i++) {
      ...
      w_DMG[i] = Math.floor(
        (w_DMG[i] * element[n_A_BodyZokusei * 10 + 1][6]) / 100
      );
    ...
    for (var i = 0; i <= 2; i++) {
      w_DMG[i] =
        baseDamage[i] * defReduction(n_B[en_HARDDEF]) -
        n_B_DEF2[i] +
        n_A_Weapon_ATK;
      w_DMG[i] *= w_SkillMod + n_A_ActiveSkillLV * 0.4;
      w_DMG[i] = Math.floor(
        (w_DMG[i] * element[n_B[en_ELEMENT]][ele_DARK]) / 100
      );
      ...
      w_DMG[i] = tPlusDamCut(
        Math.floor(((w + w_DMG[i]) * element[n_B[en_ELEMENT]][ele_DARK]) / 100)
      );
    ...
    if (monsterDebuffs[6] === 0) {
      for (var i = 0; i < 3; i++) {
        ...
        InnStr[i] +=
          Last_DMG_A[i] + " (" + w_DMG[i] + SubName[8][Language] + "3hit)";
        w_DMG[i] = Last_DMG_A[i];
      }
    } else {
      for (var i = 0; i < 3; i++) {
    ...
    wCR = 100;
    n_PerHIT_DMG = Math.floor(
      (ApplyDamageModifiers(0) * 2 * element[n_B[en_ELEMENT]][ele_NEUTRAL]) /
        100
    );
    ...
    for (var i = 0; i < 3; i++) {
      ...
      w_DMG[i] = Math.floor(
        (w_DMG[i] * element[n_B[en_ELEMENT]][ele_NEUTRAL]) / 100
      );
      Last_DMG_A[i] = Last_DMG_B[i] = w_DMG[i];
      InnStr[i] += Last_DMG_A[i];
    }
    w_DMG[1] =
      (w_DMG[1] * w_HIT + ApplyDamageModifiers(0) * 2 * (100 - w_HIT)) / 100;
    w_DMG[1] = Math.floor(
      (w_DMG[1] * element[n_B[en_ELEMENT]][ele_NEUTRAL]) / 100
    );
    ...
    w_DMG[2] = Math.floor(
      (w_DMG[2] * element[n_B[en_ELEMENT]][ele_NEUTRAL]) / 100
    );
    w_DMG[0] = w_DMG[1] = w_DMG[2];
    for (var i = 0; i < 3; i++) {
      Last_DMG_A[i] = Last_DMG_B[i] = w_DMG[i];
    ...
    else w_SkillMod = 8 + (n_A_MaxSP - 1) / 10;
    wASYU = 250 + n_A_ActiveSkillLV * 150;
    if (SkillSearch(skill_SUR_RISING_DRAGON) > 0) {
      //prime
      w_SkillMod *= 2;
    }
    for (var i = 0; i < 3; i++) {
    ...
    w_DMG[1] =
      (w_DMG[1] * w_HIT +
        ((ApplyDamageModifiers(0) * element[n_B[en_ELEMENT]][ele_NEUTRAL]) /
          100) *
          (100 - w_HIT)) /
      100;
    n_PerHIT_DMG =
      (ApplyDamageModifiers(0) * element[n_B[en_ELEMENT]][ele_NEUTRAL]) / 100;
    ...
    n_A_Weapon_element =
      KunaiOBJ[parseInt(formElements["SkillSubNum"].value)][1];
    ...
    for (var i = 0; i < 3; i++) {
      ...
      InnStr[i] +=
        Last_DMG_A[i] + " (" + Last_DMG_B[i] + SubName[8][Language] + "3hit)";
      w_DMG[i] = Last_DMG_A[i];
    }
    var wX = Math.floor(
      (ApplyDamageModifiers(0) * element[n_B[en_ELEMENT]][ele_NEUTRAL]) / 100
    ...
    for (var i = 0; i < 3; i++) {
      ...
      InnStr[i] += Last_DMG_A[i];
      InnStr[i] +=
        " (" + Last_DMG_A[i] / wActiveHitNum + " x " + wActiveHitNum + "Hit)";
    }
    w_DMG[1] =
      (w_DMG[1] * w_HIT +
        ((ApplyDamageModifiers(0) * element[n_B[en_ELEMENT]][ele_NEUTRAL]) /
    ...
    n_A_ActiveSkill === skill_NIN_KILLING_STRIKE ||
    ...
    var ks_statatk = Math.floor(
      n_A_BaseLV / 4 + n_A_STR + n_A_DEX / 5 + n_A_LUK / 3
    );
    ...
    for (var i = 0; i < 3; i++) {
      ...
      ks_damages[i] =
        (ks_atk[i] * n_A_ActiveSkillLV * remainingHP) / n_A_MaxHP + remainingHP;
    ...
    CalcAtkMods02(w_SkillMod, 0);
    if (n_tok[bon_ICE_PICK]) equipmentAttack += Math.floor(n_B[en_HARDDEF] / 2);
    var tempAttack =
      equipmentAttack +
      n_A_Weapon_ATK +
      weaponUpgradeAttack +
    ...
    for (var i = 0; i < 3; i++) {
      ...
      w_DMG[i] = tPlusDamCut(
        Math.floor(
          (w_DMG[i] * element[n_B[en_ELEMENT]][n_A_Weapon_element]) / 100
        )
      );
      Last_DMG_A[i] = Last_DMG_B[i] = w_DMG[i];
      InnStr[i] += Last_DMG_A[i];
    ...
    for (var i = 0; i < 3; i++) {
      ...
      // multiply boss mod
      tempAttack[i] *= (bossMod + 100) / 100;
      if (otherBuffs[ksMagnumBreak]) tempAttack[i] += tempAttack[i] * 0.2;
      // multiply attack mod
      tempAttack[i] *= attackMod;
      //tempAttack[i] += masteryAttack;

      wAD[i] =
        (n_A_MATK[i] * 0.07 + 0.07 * tempAttack[i] + 0.14 * statusAttack) *
        n_B[en_VIT];
    ...
    n_A_ActiveSkill == skill_HU_LAND_MINE ||
    ...
    if (n_A_ActiveSkill == skill_HU_LAND_MINE) {
      ...
      w_DMG[2] = Math.floor(
        ((n_A_DEX * (3 + n_A_BaseLV / 100) * (1 + n_A_INT / 35) +
          SkillSearch(skill_RAN_RESEARCH_TRAP)) *
          n_A_ActiveSkillLV *
          element[n_B[en_ELEMENT]][ele_EARTH]) /
          100
      ); //prime
      ...
      w_DMG[2] = Math.floor(
        ((n_A_DEX * (3 + n_A_BaseLV / 100) * (1 + n_A_INT / 35) +
          SkillSearch(skill_RAN_RESEARCH_TRAP)) *
          n_A_ActiveSkillLV *
          element[n_B[en_ELEMENT]][ele_WIND]) /
          100
      ); //prime
      ...
      w_DMG[2] = Math.floor(
        ((n_A_DEX * (3 + n_A_BaseLV / 100) * (1 + n_A_INT / 35) +
          SkillSearch(skill_RAN_RESEARCH_TRAP)) *
          n_A_ActiveSkillLV *
          element[n_B[en_ELEMENT]][ele_FIRE]) /
          100
      );
    }
    ...
    w_DMG[2] = Math.floor(
      (Math.floor(w_DMG[2] / 2) * element[n_B[en_ELEMENT]][ele_HOLY]) / 100
    );
    ...
    w_DMG[2] = Math.floor(
      (Math.floor(w_DMG[2] / 2) * element[n_B[en_ELEMENT]][ele_DARK]) / 100
    );
    if (n_B[en_ELEMENT] < 90 && n_B[en_RACE] != 6) w_DMG[2] = 0;
    ...
    n_A_ActiveSkill == skill_PR_TURN_UNDEAD ||
    n_A_ActiveSkill == skill_PR_RESSURECTION
  ) {
    n_PerHIT_DMG = 0;
    ...
    if (n_B[en_ELEMENT] < 90) {
      ...
      w_DMG[0] = Math.floor(
        (w_DMG[0] * element[n_B[en_ELEMENT]][n_A_Weapon_element]) / 100
      );
    ...
    InnStr[2] += Math.floor(
      (w_DMG[2] * element[n_B[en_ELEMENT]][n_A_Weapon_element]) / 100
    );
    ...
    if (PATCH == 0) {
      ...
      n_A_DMG = GetBaseDmg(n_A_Weapon_element, false, 0);
      w_SkillMod =
        ((strikingLevel * 0.5 + n_A_ActiveSkillLV * 0.5) * n_A_BaseLV) / 100.0;
      for (var i = 0; i < 3; i++) {
        // Apply Enemy Defense
      ...
      w_SkillMod =
        ((endowLevel * 0.5 + (n_A_ActiveSkillLV * n_A_INT) / 100.0) *
          n_A_BaseLV) /
        100.0;
      for (var i = 0; i < 3; i++) {
        // Apply Enemy Magic Defence
        magicDamage[i] = CalcMagicDamage(
          CalcMATKMultipliers(n_A_MATK[i]) * w_SkillMod
        );
      }
      myInnerHtml("CRInum", magicDamage[0] + "-" + magicDamage[2], 0);
      ...
      w_SkillMod =
        (((n_A_ActiveSkillLV * n_A_INT) / 200.0 +
          endowLevel * 1.2 +
          strikingLevel * 1.2) *
          n_A_BaseLV) /
        100.0;
      ...
      w_SkillMod =
        (((n_A_ActiveSkillLV + 2) * (n_A_INT / 200.0) +
          (endowLevel + strikingLevel) * 1.5) *
          n_A_BaseLV) /
        100.0;
      for (var i = 0; i < 3; i++) {
        // Apply Enemy Magic Defence
    ...
    for (var i = 0; i < 3; i++) {
      w_DMG[i] =
        ((physicalDamage[i] + magicDamage[i]) * numReverbs) / numEnemies;
      Last_DMG_A[i] = w_DMG[i];
      InnStr[i] += Math.floor(Last_DMG_A[i]) + " (";
      InnStr[i] +=
        Math.floor((physicalDamage[i] * numReverbs) / numEnemies) + "-";
    ...
    if (PATCH == 2) {
      ...
      else w_SkillMod = n_A_ActiveSkillLV + (n_A_DEX + n_A_AGI) / 200;
    } else {
      if (PATCH == 1) {
        if (
          n_A_WeaponType == weapTyp_INSTRUMENT ||
          ...
          w_SkillMod =
            ((n_A_DEX + n_A_AGI) * (n_A_ActiveSkillLV / 5) * 1.5) / 100.0;
        else w_SkillMod = ((n_A_DEX + n_A_AGI) * n_A_ActiveSkillLV) / 5 / 100.0;
      } else w_SkillMod = ((n_A_DEX + n_A_AGI) * n_A_ActiveSkillLV) / 5 / 100.0;
    }
    w_SkillMod *= n_A_BaseLV / 100.0;
    CalcAtkMods02(w_SkillMod, 0);
    ...
    for (var i = 0; i < 3; i++) {
      ...
      InnStr[i] =
        Math.floor(w_DMG[i]) + " (" + physicalDamage[i] + " x " + numHits + ")";
    }
    ...
    if (PATCH < 2) {
      ...
      dragonTrainingMod =
        (95 + 5 * SkillSearch(skill_RUN_DRAGON_TRAINING)) / 100.0;
      // Damage = (currentHP / 50 + MSP / 4) * (SkillLevel * BaseLvl / 150) * DragonTrainingBonus
      baseDamage =
        (currentHP / 50.0 + n_A_MaxSP / 4.0) *
        ((n_A_ActiveSkillLV * n_A_BaseLV) / 150.0) *
        dragonTrainingMod;
    } else if (PATCH == 2) {
      // Dragon Breath damage bonus: (90 + 10 * Skill Level) %
      dragonTrainingMod =
        (90 + 10 * SkillSearch(skill_RUN_DRAGON_TRAINING)) / 100.0;
      // Damage = (currentHP / 50 + MSP / 4) * (SkillLevel * BaseLvl / 100) * DragonTrainingBonus
      baseDamage =
        (currentHP / 50.0 + n_A_MaxSP / 4.0) *
        ((n_A_ActiveSkillLV * n_A_BaseLV) / 100.0) *
        dragonTrainingMod;
    }
    ...
    if (SkillSearch(skill_RUN_DRAGON_TRAINING) > 0) {
      totalDamage = Math.floor(
        baseDamage * rangeddamageMod * demiHumanMod * elementalMod
      );
    }
    for (var i = 0; i < 3; i++) {
      w_DMG[i] = totalDamage;
    ...
    var totalDamage =
      n_A_ActiveSkillLV * 1000 +
      n_A_INT * SkillSearch(skill_MIWA_VOICE_LESSONS);
    ...
    var totalDamage =
      n_B[en_VIT] * (SkillSearch(skill_GLT_RESEARCH_NEW_POISON) - 3) +
      n_B[en_HP] / 100;
    ...
    if (PATCH == 0) {
      ...
      w_SkillMod =
        ((n_A_ActiveSkillLV * 1.5 + (n_A_STR + shieldWeight) / 100.0) *
          n_A_BaseLV) /
        100.0;
      ...
      w_SkillMod =
        ((n_A_ActiveSkillLV * 2.0 + (n_A_STR + shieldWeight) / 100.0) *
          n_A_BaseLV) /
        100.0;
    }
    ...
    if (n_A_HEAD_DEF_PLUS >= 9) HolyMagicMul += CardNumSearch(845) * 5;
    if (PATCH == 0) {
      // Calculate Physical portion
      ...
      //CalcAtkMods02( w_SkillMod, 0 );
      for (var i = 0; i < 3; i++) {
        physicalDamage[i] = Math.floor(
          ApplyEnemyDefense(n_A_DMG[i] * w_SkillMod, i, 0)
        );
      }
      myInnerHtml("CRIATK", physicalDamage[0] + "-" + physicalDamage[2], 0);
    ...
    variableCastTime *= 1.5 + 0.5 * n_A_ActiveSkillLV;
    n_Delay[ksDelayGlobal] = 2.0;
    if (n_A_ActiveSkill >= 5 && PATCH >= 1) n_Delay[ksDelayGlobal] = 1.0;
    n_Delay[ksDelayCooldown] = 5.0;
    ...
    for (var i = 0; i < 3; i++) {
      physicalDamage[i] =
        skillMod * remodelMod * spMod * baseLevelMod + currentHP;
      physicalDamage[i] = ApplyEnemyDefense(physicalDamage[i], i, 0);
    }
    ...
    if (PATCH < 2) {
      ...
      w_SkillMod =
        (((n_A_ActiveSkillLV * 3.0 * n_A_BaseLV) / 100.0) * 4.0) / 5.0;
      n_Delay[ksDelayGlobal] = 1.0;
    } else if (PATCH == 2) {
    ...
    if (PATCH == 0) {
      ...
      for (var i = 0; i < 3; i++) {
        physicalDamage[i] = ApplyEnemyDefense(n_A_DMG[i] * w_SkillMod, i, 0);
        physicalDamage[i] +=
          (monsterSizeValue + n_A_ActiveSkillLV - 1) * n_A_STR +
          (n_B[en_LEVEL] * 50 * n_A_DEX) / 120.0;
      }
    } else if (PATCH == 1) {
    ...
    for (var i = 0; i < 3; i++) {
      ...
      if (afterCombo) {
        ...
        physicalDamage[i] +=
          n_A_MaxHP - currentHP + n_A_MaxSP * spMod + n_A_BaseLV * 40;
      } else {
        // (Caster s Max HP - Caster s Current HP) + (Caster s SP x Damage modifier) + (Caster s Base Level x 10
        physicalDamage[i] +=
          n_A_MaxHP - currentHP + currentSP * spMod + n_A_BaseLV * 10;
      }
    }
    ...
    w_SkillMod =
      ((n_B[en_HP] / 100.0 / 100.0) * n_A_ActiveSkillLV * n_A_BaseLV) / 125.0;
    if (w_SkillMod > 50.0) {
      w_SkillMod = 50.0;
    }
    //CalcAtkMods02( w_SkillMod, 0 );
    ...
    for (var i = 0; i <= 2; i++) {
      ...
      HMEDamage[i] = CalcFinalDamage(
        n_A_DMG[i] * MineExplosionDamageSkillMod,
        i
      );
    ...
    for (var i = 0; i < 3; i++) {
      ...
      InnStr[i] +=
        Math.floor(HMDamage[i]) + " + " + HMEDamage[i] + " explosion damage";
    }
    ...
    if (EquipNumSearch(2245)) {
      //Avenger [2]
      if (n_A_Weapon_ATKplus >= 7) n_Delay[ksDelayCooldown] -= 1.0;
    }
    ...
    for (var i = 0; i < 3; i++) {
      ...
      InnStr[i] +=
        Math.floor(BTDamage[i]) + " + " + BTKBDamage[i] + " knockback damage";
    }
    ...
    var BTDamage = [0, 0, 0]; // min, ave, max
    for (var i = 0; i <= 2; i++) {
      BTDamage[i] = Math.floor(
        (n_B[en_HP] / 100) * (3 * n_A_ActiveSkillLV) + n_A_DEX * 10
      );
    ...
    for (var i = 0; i < 3; i++) {
      physicalDamage[i] = ApplyEnemyDefense(n_A_DMG[i] * w_SkillMod, i, 0);
      physicalDamage[i] +=
        (monsterSizeValue + FallenEmpireLv - 1) * n_A_STR +
        (n_B[en_LEVEL] * 50 * n_A_DEX) / 120.0;
    }
    ...
    for (var i = 0; i < 3; i++) {
      ...
      if (EquipNumSearch(2074)) {
        ...
        if (n_A_Weapon_ATKplus <= 8)
          ...
          n_A_DMG[i] *=
            (100 + 2 * Math.floor(n_A_Weapon_ATKplus / 3) + 15) / 100;
      }
      physicalDamage[i] = ApplyEnemyDefense(n_A_DMG[i] * w_SkillMod, i, 0);
    ...
    for (var i = 0; i < 3; i++) {
      w_DMG[i] =
        DragonCombo[i] + FallenEmpire[i] + TigerCannon[i] + SkyNetBlow[i];
      Last_DMG_A[i] = w_DMG[i];
      InnStr[i] += Math.floor(Last_DMG_A[i]);
      InnStr[i] +=
        '<input type="button" value="Show details" onclick="javascript:$(\'#FlashCombo' +
        i +
        '\').toggle()"><span style="display: none;" id="FlashCombo' +
        i +
        '"><br>';
      InnStr[i] +=
        "Dragon Combo : " +
  ...
  // Magic Skills ----------------------------------
    ...
    // Wizard Skills
      ...
      n_A_ActiveSkill == skill_WI_WATER_BALL ||
      n_A_ActiveSkill == skill_WATER_BALL2
    ) {
      n_A_Weapon_element = ele_WATER;
      if (n_A_ActiveSkillLV == 1) w_TotalHits = 1;
      else if (n_A_ActiveSkillLV >= 4) w_TotalHits = 25;
      else if (n_A_ActiveSkillLV >= 2) w_TotalHits = 9;
      ...
      n_A_ActiveSkill == skill_WI_EARTH_SPIKE ||
    ...
    // Acolyte Skills
      ...
      n_A_Weapon_element = ele_HOLY;
      w_SkillMod = 1.25;
      if (n_A_ActiveSkill == skill_PR_HOLY_LIGHT_SL) w_SkillMod *= 5;
    ...
    // Soul Linker Skills
      ...
      n_A_Weapon_element = eval(document.calcForm.A_Weapon_element.value);
      if (n_B[en_SIZE] == 0) w_SkillMod = n_A_ActiveSkillLV * 0.1;
      else w_SkillMod = 0.01;
      if (PlayerVersusPlayer == 1) w_SkillMod = 0;
    ...
    // Warlock Skills
      ...
      w_SkillMod =
        ((n_A_ActiveSkillLV * 2.0 + n_A_INT / 100.0) * n_A_BaseLV) / 100.0;
      w_TotalHits = 1;
      ...
      if (PATCH < 2) {
        ...
        w_SkillMod =
          ((n_A_ActiveSkillLV + 4 + n_A_INT / 100.0) * n_A_BaseLV) / 100.0;
      } else if (PATCH == 2) {
        // MATK [{( Skill Level x 200) x 1000 ) + ( Caster s INT )} x ( Caster s Base Level / 100 )] %
        w_SkillMod =
          ((n_A_ActiveSkillLV * 2.0 + 10.0 + n_A_INT / 100.0) * n_A_BaseLV) /
          100.0;
      }
      ...
      if (PATCH < 2) {
        ...
        w_SkillMod =
          (n_A_ActiveSkillLV * 4.0 * n_A_BaseLV) / 120.0 + distanceMod;
      } else if (PATCH == 2) {
      ...
      if (PATCH < 2) w_TotalHits = 1;
      else if (PATCH == 2) w_TotalHits = n_A_ActiveSkillLV;
      ...
      variableCastTime *= 1.5 + 0.5 * n_A_ActiveSkillLV;
      n_Delay[ksDelayGlobal] = 1.0;
      if (EquipNumSearch(2064) && n_A_Weapon_ATKplus >= 11)
      ...
      for (i = 0; i < w_TotalHits; i++) {
        w_SkillMod +=
          ((n_A_ActiveSkillLV + 5.0 + (9.0 - i)) * n_A_BaseLV) / 100.0;
      }
      ...
      if (EquipNumSearch(2063) && n_A_Weapon_ATKplus >= 11)
        //Rusty Dragon's Wand
        n_Delay[ksDelayCooldown] -= 1;
    ...
    // Arch Bishop Skills
      ...
      if (PATCH == 0) {
        ...
        if (SkillSearch(skill_ABI_ANCILLA)) n_A_Weapon_element = ele_NEUTRAL;
        else n_A_Weapon_element = ele_HOLY;
    ...
    // Sorcerer Skills
      ...
      n_A_ActiveSkill == skill_SOR_FIRE_WALK ||
      n_A_ActiveSkill == skill_SOR_ELECTRIC_WALK
    ) {
      ...
      if (PATCH < 2) {
        ...
        w_SkillMod =
          ((n_A_ActiveSkillLV * (n_A_INT / 100.0) +
            SkillSearch(skill_SA_ENDOW_EARTH) * 2.0) *
            n_A_BaseLV) /
          100.0 /
          3.0;
      } else if (PATCH == 2) {
        ...
        w_SkillMod =
          (((n_A_ActiveSkillLV + 2) * (n_A_INT / 100.0) +
            SkillSearch(skill_SA_ENDOW_EARTH) * 3.0) *
            n_A_BaseLV) /
          100.0 /
          3.0;
      }
      ...
      if (PATCH < 2) {
        ...
        w_SkillMod =
          ((n_A_ActiveSkillLV * (n_A_INT / 100.0) +
            SkillSearch(skill_SA_ENDOW_WATER) * 2.0) *
            n_A_BaseLV) /
          100.0 /
          5.0;
      } else if (PATCH == 2) {
        ...
        w_SkillMod =
          (((n_A_ActiveSkillLV + 2) * (n_A_INT / 100.0) +
            SkillSearch(skill_SA_ENDOW_WATER) * 3.0) *
            n_A_BaseLV) /
          100.0 /
          5.0;
      }
      ...
      n_A_Weapon_element = ele_NEUTRAL;
      w_SkillMod =
        ((n_A_ActiveSkillLV * 0.7 + n_A_INT * 0.03) * n_A_BaseLV) / 100.0;
      if (
      ...
      var bonus =
        (100 +
          StPlusCalc2(bon_DMG_SKILL + skill_MA_FIRE_BOLT) +
          StPlusCard(bon_DMG_SKILL + skill_MA_FIRE_BOLT) +
          StPlusEnchant(bon_DMG_SKILL + skill_MA_FIRE_BOLT) +
          MNC) /
        100.0;
      if (PATCH < 2) {
      ...
      var bonus =
        (100 +
          StPlusCalc2(bon_DMG_SKILL + skill_MA_COLD_BOLT) +
          StPlusCard(bon_DMG_SKILL + skill_MA_COLD_BOLT) +
          StPlusEnchant(bon_DMG_SKILL + skill_MA_COLD_BOLT) +
          MNC) /
        100.0;
      if (PATCH < 2) {
        // MATK [(100 * Bolt Skill Level) + (50 * Skill Level)] %
      ...
      var bonus =
        (100 +
          StPlusCalc2(bon_DMG_SKILL + skill_MA_LIGHTNING_BOLT) +
          StPlusCard(bon_DMG_SKILL + skill_MA_LIGHTNING_BOLT) +
          StPlusEnchant(bon_DMG_SKILL + skill_MA_LIGHTNING_BOLT) +
          MNC) /
        100.0;
      if (PATCH < 2) {
        // MATK [(100 * Bolt Skill Level) + (50 * Skill Level)] %
    ...
    // Performer Skills
      ...
      n_A_Weapon_element = ele_NEUTRAL;
      w_SkillMod =
        ((n_A_ActiveSkillLV * 1.2 +
          SkillSearch(skill_MIWA_VOICE_LESSONS) * 0.6) *
          n_A_BaseLV) /
        100.0; // skill mod
      n_subHits = true;
      if (PATCH == 1 || PATCH == 2) w_TotalHits = 2;
      else {
        if (n_A_ActiveSkillLV == 1 || n_A_ActiveSkillLV == 2) w_TotalHits = 2;
      ...
      if (EquipNumSearch(1902)) {
        ...
        w_SkillMod +=
          ((Math.floor(SU_INT / 6) * SV_StemLv) / 100) * EquipNumSearch(1902);
    ...
    if (n_subHits === 0) {
      ...
      for (var i = 0; i < 3; i++) {
        ...
        if (
          EquipNumSearch(1744) &&
          n_A_ActiveSkill == skill_WI_JUPITEL_THUNDER
        ) {
          w_DMG[i] += Math.floor(w_DMG[i] * (n_A_SHADOW_BODY_DEF_PLUS * 0.05));
        }
        ...
        if (n_A_ActiveSkill == skill_ROY_SHIELD_SPELL_MATK) {
          var shieldMdef = 0;
          for (
            var j = itm_BONUS_START;
            ItemOBJ[n_A_Equip[eq_SHIELD]][j] !== bon_NONE;
            j += 2
          ) {
            // find shield MDEF
          ...
          if (shieldMdef != 0) {
            ...
            Last_DMG_A[i] = 0;
            InnStr[i] +=
              '<span style="color: red">Shield doesn\'t have Mdef</span>';
            w_DMG[i] = Last_DMG_A[i];
          }
        } else {
          Last_DMG_B[i] = w_DMG[i];
      ...
      for (var i = 0; i < 3; i++) {
        w_DMG[i] = Math.floor(
          CalcMagicDamage(CalcMATKMultipliers(n_A_MATK[i]) * w_SkillMod) /
            w_TotalHits
        );
        Last_DMG_A[i] = Last_DMG_B[i] = w_DMG[i] * w_TotalHits;
        InnStr[i] +=
      ...
      if (n_A_ActiveSkill == skill_MIWA_METALLIC_SOUND) {
        ...
        myInnerHtml(
          "CRInumname",
          '<font color="#0000FF">SP drained from enemy</font>',
          0
        );
        ...
        myInnerHtml(
          "CRInum",
          '<font color="#0000FF">' + spDrainLow + "-" + spDrainHigh + "</font>",
          0
        );
      }
    }
  ...
  var delayFixRed =
    StPlusCalc2(bon_DELAY_SKILL_FLAT + n_A_ActiveSkill) +
    StPlusCard(
      bon_DELAY_SKILL_FLAT + n_A_ActiveSkill
    ); /*+ StPlusEnchant(bon_DELAY_SKILL_FLAT+n_A_ActiveSkill)*/
  n_Delay[ksDelayCooldown] = Math.max(
    n_Delay[ksDelayCooldown] - delayFixRed,
    0
  );
  var castTimeFixRed =
    StPlusCalc2(bon_CAST_SKILL_FLAT + n_A_ActiveSkill) +
    StPlusCard(bon_CAST_SKILL_FLAT + n_A_ActiveSkill);
...
function setupPVPPlayerMods() {
  ...
  if (n_B[en_ID] === PVP_ENEMY && monster.length >= 27) {
    monsterBuffs[status_en_buff_Race] = document.getElementsByName(
      "B_KYOUKA10"
    )[0].value = monster[26];
    monsterBuffs[status_en_buff_Elemental] = document.getElementsByName(
      "B_KYOUKA11"
    )[0].value = monster[32 + n_A_Weapon_element];
    monsterBuffs[status_en_buff_Ranged] = document.getElementsByName(
      "B_KYOUKA12"
    )[0].value = monster[27];
    monsterBuffs[status_en_buff_Size] = document.getElementsByName(
      "B_KYOUKA13"
    )[0].value = monster[28];
    monsterBuffs[status_en_buff_Normal] = document.getElementsByName(
      "B_KYOUKA14"
    )[0].value = monster[29];
    monsterBuffs[status_en_buff_Other] = document.getElementsByName(
      "B_KYOUKA15"
    )[0].value = monster[30];
  }
}
